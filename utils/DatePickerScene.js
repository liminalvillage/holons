...
export default class DatePickerScene {
        ...
        this.scene.enter(async (ctx) => {
            try {
                ...
                if (state.dateConfig?.minDate) {
                    calendarOptions.start_date = this.resolveDate(state.dateConfig.minDate, ctx);
                }
                if (state.dateConfig?.maxDate) {
                    calendarOptions.stop_date = this.resolveDate(state.dateConfig.maxDate, ctx);
                }

                // Store calendar instance in session
                ctx.session.datePickerCalendar = new Calendar(ctx.telegraf, calendarOptions);
                ctx.session.datePickerCalendar.startNavCalendar(ctx, calendarOptions.language);
        ...
        this.scene.on('callback_query', async (ctx) => {
            try {
                ...
                if (data && ctx.session.datePickerCalendar) {
                    const result = ctx.session.datePickerCalendar.clickButtonCalendar(ctx);
    ...
    resolveDate(dateSpec, ctx) {
        ...
        console.warn(`DatePickerScene: Could not parse date spec "${dateSpec}", defaulting to today`);
        return new Date();
    }
}
