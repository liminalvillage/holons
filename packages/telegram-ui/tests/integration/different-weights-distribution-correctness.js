...
import managed from '../artifacts/ManagedABI.json' with { type: 'json' };
import splitter from '../artifacts/SplitterABI.json' with { type: 'json' };
import zoned from '../artifacts/ZonedABI.json' with { type: 'json' };
import appreciative from '../artifacts/AppreciativeABI.json' with { type: 'json' };
import holonsFactoryAbi from '../artifacts/HolonsFactoryABI.json' with { type: 'json' };
...
async function runTest(contract, holonId, testCase) {
  ...
  console.log(
    `Expected distribution: ${testCase.expectedRatio.map(r => `${(r * 100).toFixed(1)}%`).join(' / ')}`
  );
  users.forEach((u, i) => {
    const actual = after[u.id] - before[u.id];
    const expected =
      (TOTAL_REWARD * BigInt(Math.floor(testCase.expectedRatio[i] * 1e6))) /
      1_000_000n;
    const diff = actual > expected ? actual - expected : expected - actual;
    const ok = diff < ethers.parseEther('0.0001');
    console.log(
      `💸 ${u.id} got ${ethers.formatEther(actual)} ETH (expected ~${ethers.formatEther(expected)} ETH)`
    );
...
const runAll = async () => {
  ...
  if (holonAddress === ethers.ZeroAddress)
    throw new Error(`No holon for ${holonId}`);
