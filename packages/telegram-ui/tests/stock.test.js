...
describe('parseStockCommand', () => {
  it('bare /stock is the shelf, an unknown word is help', () => {
    ...
    expect(parseStockCommand('/stock@HolonsBot')).toEqual({ sub: 'shelf', rest: '' });
  ...
  it('accepts the Italian and Spanish verbs', () => {
    ...
    expect(parseStockCommand('/existencias objetivo 10 harina').sub).toBe('target');
  });

  it('reorder carries its buy flag', () => {
    expect(parseStockCommand('/stock reorder')).toEqual({ sub: 'reorder', rest: '' });
    expect(parseStockCommand('/stock reorder BUY')).toEqual({ sub: 'reorder', rest: '', flag: 'buy' });
  });
});

describe('parseQuantityLine', () => {
  it('reads glued and spaced units, decimal commas, and a note', () => {
    expect(parseQuantityLine('5kg flour')).toEqual({ quantity: 5, unit: 'kg', item: 'flour' });
    ...
    expect(parseQuantityLine('12 pcs screws')).toEqual({ quantity: 12, unit: 'one', item: 'screws' });
  });
  ...
  it('a word glued to the number that is not a unit stays part of the item', () => {
    ...
    expect(parseQuantityLine('2eggs of the farm')).toEqual({ quantity: 2, item: 'eggs of the farm' });
  });

  it('uses known item names to split the note off', () => {
    expect(parseQuantityLine('3 olive oil for the kitchen', ['Olive oil', 'Flour'])).toEqual({
      quantity: 3,
      item: 'olive oil',
...
describe('buildBoard', () => {
  ...
  it('a need short here is covered from a partner surplus, one hop away', () => {
    const board = buildBoard({
      ...
      needs: [{ type: 'need', status: 'requested', category: 'food', stock: { itemId: 'flour', quantity: 3 } }],
      federated: [P],
      partners: [
        { id: P, name: 'Mill', specs: [spec('flour', { min: 2 })], events: [produced(P, 'flour', 10)], federated: [H] },
      ],
    });
    expect(board.levels[0]).toMatchObject({ itemId: 'flour', onhand: 1, reserved: 1 });
    ...
    expect(board.plan).toEqual([{ from: P, to: H, category: 'food', quantity: 2, cost: 1 }]);
  });
});

describe('Stock feature', () => {
  beforeAll(async () => {
    const en = JSON.parse(readFileSync(new URL('../data/locales/en.json', import.meta.url), 'utf8'));
  ...
  it('registers /stock with its aliases and the reorder action', () => {
    ...
    expect(bot.action).toHaveBeenCalledWith('stock_reorder_buy', expect.any(Function));
  });

  it('replies with the empty hint when nothing is on the shelf', async () => {
    const bot = { command: vi.fn(), action: vi.fn() };
    const db = { get: vi.fn(), getAll: vi.fn(async () => []), put: vi.fn(), delete: vi.fn() };
    const stock = new Stock(bot, db, { getLanguage: async () => 'en' });
    const reply = vi.fn(async () => {});
    await stock.handle({ chat: { id: -1 }, from: { id: 1 }, message: { text: '/stock' }, reply });
