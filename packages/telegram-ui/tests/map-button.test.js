...
class MockSettings {
  ...
  async showHexMenu(ctx, edit = false) {
    const holonId =
      ctx.callbackQuery?.message?.chat?.id || ctx.chat?.id || 'test-chat-123';
...
describe('Map Hexamap Webapp Button', () => {
  ...
  it('should add map button to hex menu', async () => {
    ...
    expect(mapButton[0].web_app.url).toBe(
      'https://hexamap.holons.io/index.html?id=test-chat-123'
    );
  });
  ...
  it('should include all required buttons in hex menu', async () => {
    ...
    const buttonTexts = keyboard.inline_keyboard
      .flat()
