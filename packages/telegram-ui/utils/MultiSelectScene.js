...
export default class MultiSelectScene {
  ...
  setupScene() {
    ...
    this.scene.action('mss_done', async ctx => {
      try {
        ...
        // Get selected values
        const selected = Object.keys(ctx.session.mss.selected).filter(
          k => ctx.session.mss.selected[k]
        );

        // Validate minimum selections
        if (state.minSelections && selected.length < state.minSelections) {
          await ctx.reply(
            `Please select at least ${state.minSelections} option(s). You have selected ${selected.length}.`
          );
          return;
        }
        ...
        if (state.maxSelections && selected.length > state.maxSelections) {
          await ctx.reply(
            `Please select no more than ${state.maxSelections} option(s). You have selected ${selected.length}.`
          );
          return;
        }
  ...
  createKeyboard(ctx, state) {
    ...
    pageOptions.forEach(opt => {
      ...
      buttons.push([
        Markup.button.callback(prefix + opt.label, `mss_toggle_${opt.value}`),
      ]);
    });
    ...
    if (totalPages > 1) {
      ...
      navRow.push(
        Markup.button.callback(`${page + 1}/${totalPages}`, 'mss_noop')
      );
