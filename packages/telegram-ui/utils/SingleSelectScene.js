...
export default class SingleSelectScene {
        ...
        if (value === '__other__' && state.allowOther) {
          ...
          return ctx.scene.enter('input_scene', {
            ...
            onComplete: async (inputCtx, input) => {
              await state.onComplete(inputCtx, {
                value: 'other',
                customValue: input,
              });
            },
    ...
    this.scene.action(/sss_info_(.+)/, async ctx => {
      try {
        ...
        if (state.infoDescriptions && state.infoDescriptions[value]) {
          await ctx.answerCbQuery(state.infoDescriptions[value], {
            show_alert: true,
          });
        } else {
  ...
  createButtons(state) {
    ...
    state.options.forEach(option => {
      const row = [
        Markup.button.callback(option.label, `sss_select_${option.value}`),
      ];

      // Add info button if descriptions are available
      if (
        state.showInfoButtons &&
        state.infoDescriptions &&
        state.infoDescriptions[option.value]
      ) {
    ...
    if (state.allowOther) {
      buttons.push([
        Markup.button.callback('Other (specify)', 'sss_select___other__'),
      ]);
    }
