...
async function uploadSchema(filename) {
    try {
        ...
        await new Promise((resolve, reject) => {
            gunUser.get('schemas').get(schemaName).put(schemaWithMeta, (ack) => {
                if (ack.err) {
...
async function uploadSchemaRegistry(uploadedSchemas) {
    try {
        const registry = {
            ...
            schemas: uploadedSchemas.filter(s => s.success).map(s => ({
                name: s.name,
                path: `schemas/${s.name}`,
...
async function uploadAllSchemas() {
    ...
    try {
        ...
        const schemaFiles = fs.readdirSync(schemasDir)
            ...
            .filter(file => !file.includes('_schema') || file.includes('quest') || file.includes('holon')); // Include our new schemas
