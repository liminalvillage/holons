...
**Why this exists:** The `POST /refresh/quest` and `POST /refresh/expense` endpoints (called from harvest after a user edits a task or expense, so the matching Telegram message is re-rendered) currently run with **no authentication** — only IP rate limiting (`Server.js` `setupSecurityMiddleware`, 100 req/min).

A shared-secret approach was considered and rejected: harvest is a browser SPA, so any token shipped in its bundle is visible in DevTools and is therefore not a secret.
...
Telegram's [Login Widget](https://core.telegram.org/widgets/login) returns a payload signed with the bot's token. Harvest already uses this flow (see `telegramUser` in `website/src/lib/Quests.svelte` and `harvest/src/lib/stores/telegram.ts`). Server-side we can verify the signature without ever shipping a secret to the browser:

1. **Harvest sends the user's auth payload alongside the refresh request:**
   ```
   POST /refresh/quest
...
3. **Reject stale tokens** — require `auth_date` within the last ~24 h (or shorter; refresh on the harvest side).
...
**Trigger:** Do this before exposing `/refresh/*` to traffic outside the harvest UI, or as soon as someone reports unwanted edits.
