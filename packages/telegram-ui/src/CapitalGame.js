...
class CapitalGame {
  constructor(bot, settings) {
    ...
    // Trade flow handlers
    this.bot.action(/trade_select_(.+)/, ctx =>
      this.handleTradeSelectCapital(ctx)
    );
    this.bot.action(/trade_target_(.+)/, ctx =>
      this.handleTradeSelectTarget(ctx)
    );
    this.bot.action(/trade_request_(.+)/, ctx =>
      this.handleTradeSelectRequest(ctx)
    );
    ...
    this.bot.action(/goal_contribute_(.+)/, ctx =>
      this.handleGoalContribution(ctx)
    );
    ...
    const events = [
      {
        ...
        effect: { type: 'financial', amount: -2 },
        description: 'Market instability affects everyone',
      },
      {
        name: 'Cultural Festival',
        effect: { type: 'cultural', amount: 2 },
        description: 'A celebration brings the community together',
      },
      {
        name: 'Environmental Challenge',
        effect: { type: 'living', amount: -1 },
        description: 'Natural resources become scarce',
      },
      {
        name: 'Knowledge Sharing',
        effect: { type: 'intellectual', amount: 2 },
        description: 'A wise teacher visits the community',
      },
      {
        name: 'Social Gathering',
        effect: { type: 'social', amount: 2 },
        description: 'Community bonds strengthen',
      },
      {
        name: 'Spiritual Awakening',
        effect: { type: 'spiritual', amount: 2 },
        description: 'A moment of collective reflection',
      },
      {
        name: 'Material Abundance',
        effect: { type: 'material', amount: 2 },
        description: 'Resources become plentiful',
      },
      {
        name: 'Adventure Opportunity',
        effect: { type: 'experiential', amount: 2 },
        description: 'New experiences await',
      },
    ];
  ...
  async startGame(ctx) {
    ...
    if (this.games.has(holonId)) {
      return ctx.reply(
        'A game is already in progress in this chat! Use /endgame to end it first.'
      );
    }
    ...
    try {
      ...
      if (!game) {
        return ctx.answerCbQuery(
          'No game is currently active. Start a new game with /capital.'
        );
      }

      if (game.status !== 'waiting') {
        return ctx.answerCbQuery(
          'Game already in progress! Wait for the next game.'
        );
      }
  ...
  async startRound(ctx) {
    ...
    await this.bot.telegram.sendMessage(
      holonId,
      'The game has started! Round 1 begins now.'
    );
  ...
  async handleTradeSelectCapital(ctx) {
    ...
    const otherPlayers = Array.from(game.players.entries()).filter(
      ([id]) => id !== userId
    );
  ...
  async handleTradeSelectTarget(ctx) {
    ...
    const availableCapitals = this.capitalTypes.filter(
      c => targetPlayer.capitals[c] >= 1
    );

    const buttons = availableCapitals.map(capital =>
      Markup.button.callback(
        `${capital} (${targetPlayer.capitals[capital]})`,
        `trade_request_${capital}`
      )
    );
    buttons.push(Markup.button.callback('Cancel', 'trade_cancel'));
    ...
    await ctx.editMessageText(
      ...
      Markup.inlineKeyboard([
        [
          Markup.button.callback(
            'Send Trade Request',
            `trade_confirm_${pendingTrade.targetUserId}`
          ),
        ],
  ...
  async handleTradeAccept(ctx) {
    ...
    if (initiator.capitals[pendingTrade.offerCapital] < 2) {
      ...
      return ctx.editMessageText(
        'Trade cancelled: Initiator no longer has enough capital.'
      );
    }
    if (target.capitals[pendingTrade.requestCapital] < 1) {
      ...
      await ctx.answerCbQuery('Trade failed!');
      return ctx.editMessageText(
        'Trade cancelled: You no longer have enough capital.'
      );
    }
  ...
  async handleTradeDecline(ctx) {
    ...
    if (
      userId !== pendingTrade.targetUserId &&
      userId !== pendingTrade.initiatorId
    ) {
  ...
  async applyRandomEvent(holonId) {
    ...
    for (const [_, player] of game.players) {
      ...
      player.capitals[capitalType] = Math.max(
        0,
        player.capitals[capitalType] + change
      );
    }
  ...
  checkWinConditions(game) {
    ...
    const goalsCompleted = game.communityGoals.every(goal =>
      this.isGoalComplete(goal)
    );
    if (goalsCompleted) {
  ...
  isGoalComplete(goal) {
    return Object.entries(goal.requirements).every(
  ...
  async announceWinners(holonId, winCondition) {
    ...
    if (winCondition.type === 'individual') {
      message =
        `Congratulations ${winCondition.winner.username}!\n\n` +
  ...
  async showTradeMenu(ctx) {
    ...
    const tradeableCapitals = this.capitalTypes.filter(
      c => player.capitals[c] >= 2
    );
    ...
    const buttons = tradeableCapitals.map(capital =>
      Markup.button.callback(
        `${capital} (${player.capitals[capital]})`,
  ...
  async showGoalsMenu(ctx) {
    ...
    const buttons = game.communityGoals.map((goal, index) => {
      const progress = Object.entries(goal.requirements).reduce(
        (sum, [cap, req]) => sum + Math.min(goal.progress[cap] || 0, req),
        0
      );
      const total = Object.values(goal.requirements).reduce(
        (sum, v) => sum + v,
        0
      );
