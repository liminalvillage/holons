  ...
  setupServer() {
    ...
    if (isDebug) {
      this.serverInstance = app.listen(port, () => {
        console.log(
          `GunServer HTTP Server running on port ${port} (debug mode)`
        );
      });
    } else {
      ...
      try {
        const sslOptions = {
          ...
          cert: fs.readFileSync(
            process.env.SSL_CERT_PATH || 'certs/certificate.crt'
          ),
        };
        ...
        console.error(
          'Error setting up HTTPS for GunServer. Defaulting to HTTP.',
          e.message
        );
        console.warn(
          'Falling back to HTTP for GunServer. Ensure SSL certs are correctly configured for HTTPS.'
        );
        this.serverInstance = app.listen(port, () => {
          console.log(
            `GunServer HTTP Server (fallback) running on port ${port}`
          );
        });
      }
    }
    ...
    this.serverInstance.on('error', error => {
      console.error(
        `Failed to start GunServer ${isDebug ? 'HTTP' : 'HTTPS'} server:`,
        error.message
      );
      this.serverInstance = null;
    });
    ...
    console.log(
      `GunServer initialized with ${this.serverInstance instanceof https.Server ? 'HTTPS' : 'HTTP'}`
    );
  }
}
