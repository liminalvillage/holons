...
class Scheduler {
  ...
  async addTask(ctx) {
    ...
    if (!frequency || taskDetails.length === 0) {
      ctx.reply(
        'Usage: /recurring [frequency] [task description]\nFrequencies: daily, weekly, monthly, quarterly, yearly\nUse /when to set the start time'
      );
      return;
    }
    ...
    if (!validFrequencies.includes(frequency.toLowerCase())) {
      ctx.reply(
        `Invalid frequency. Please use one of: ${validFrequencies.join(', ')}`
      );
      return;
    }
    ...
    const timeStr = new Date(task.when).toLocaleTimeString('en-US', {
      hour: '2-digit',
      minute: '2-digit',
    });
    ctx.reply(
      `Recurring task "${task.title}" scheduled ${task.frequency} starting at ${timeStr}`
    );
  }

  async scheduleTask(task, ctx) {
    const holonId = task.holonId;
    ...
    const job = new CronJob(
      ...
      async () => {
        try {
          ...
          if (originalTask.chat) originalTask[holonId] = originalTask.chat; // Create a copy to ensure we don't modify the original
          console.log(
            'Running scheduled task for holonId:',
            originalTask.holonId
          );
          ...
          const mockCtx = {
            chat: {
              // For utilities like getholonId that might look here first
              id: originalTask.holonId,
              type: null, // Type is unknown/not applicable for scheduler context at this level
            },
            message: {
              chat: {
                // To make ctx.message.chat.type checks safer and ctx.message.chat.id available
                id: originalTask.holonId,
          ...
          if (isEvent && !this.events) {
            console.error(
              'Recurring event task fired but Events module is not wired into Scheduler'
            );
            return;
          }
          ...
          if (!item) {
            console.error(
              `Failed to create recurring ${isEvent ? 'event' : 'quest'}`
            );
            return;
          }

          const lens = isEvent ? 'events' : 'quests';
          let itemHolonId = isEvent
            ? (item.holon ?? null)
            : getQuestHolon(item);
          console.log(
            `New ${isEvent ? 'event' : 'quest'} created, id:`,
            item.id,
            'holon ID:',
            itemHolonId
          );
          ...
          item.frequency =
            originalTask.frequency === undefined
              ? null
          ...
          if (!isEvent) {
            ...
            if (originalTask.checklistId) {
              try {
                const originalChecklist = await this.db.get(
                  originalTask.holonId,
                  'checklists',
                  originalTask.checklistId
                );
                if (originalChecklist) {
                  const newChecklist = {
                    id: item.id.toString(),
                    type: 'quest',
                    items: originalChecklist.items.map(i => ({
                      ...i,
                      checked: false,
                    })),
          ...
          console.log(
            `Saving ${isEvent ? 'event' : 'quest'} with holon ID:`,
            itemHolonId
          );
    ...
    try {
      ...
      try {
        ...
        console.log(
          ...
          'lookup records to check'
        );

        if (lookups && lookups.length > 0) {
          const relatedLookups = lookups.filter(
            lookup => lookup.taskID === taskId
          );
          console.log(
            'Found',
            relatedLookups.length,
            'lookup records to delete'
          );
  ...
  async updateTaskSchedule(holonId, questId, selectedDate, ctx) {
    try {
      ...
      if (quest.type === 'recurring') {
        ...
        if (recurringID) {
          const task = await this.db.getGlobal('recurring', recurringID.taskID);
          if (task) {
  ...
  async showCalendar(ctx, itemId, lens = 'quests') {
    try {
      ...
      const now = new Date();
      now.setDate(1);
      const calendarMarkup = this.calendar.createNavigationKeyboard(
        now,
        holonId
      );
  ...
  async handleCalendarNavigation(ctx) {
    try {
      ...
      await this.calendar.clickButtonCalendar(ctx);
  ...
  async handleTimeSelection(ctx) {
    try {
      ...
      const localSelectedDayjs = dayjs.tz(
        dateTimeStr,
        'YYYY-MM-DD HH:mm',
        chatTimezone
      );
      ...
      if (!localSelectedDayjs.isValid()) {
        console.error(
          `Could not parse dateTimeStr into a valid dayjs object: ${dateTimeStr} with timezone ${chatTimezone}`
        );
      ...
      if (lens === 'events' && this.events) {
        ...
        await this.events.updateEventMessage(
          ctx,
          item,
          holonId,
          calendarMsgId,
        ...
        await this.quests.updateQuestMessage(
          ctx,
          item,
          holonId,
          calendarMsgId,
  ...
  async scheduleOneTimeReminder(item, ctx, lens = 'quests') {
    try {
      ...
      if (!item.when) {
        console.error(
          `Cannot schedule reminder: no when date provided for ${lens} item`,
          item.id
        );
        return;
      }
      ...
      if (reminderDate <= now) {
        console.log(
          `Reminder date ${reminderDate} is in the past, not scheduling`
        );
        return;
      }
      ...
      if (item.reminderId) {
        ...
        console.log(
          `Cancelled existing reminder ${item.reminderId} for ${lens} ${item.id}`
        );
      }
      ...
      const job = new CronJob(
        ...
        async () => {
          try {
            ...
            if (!freshItem) {
              console.log(
                `${lens} item ${item.id} no longer exists, skipping reminder`
              );
            ...
            if (freshItem.status !== 'scheduled') {
              console.log(
                `${lens} item ${item.id} is no longer scheduled (status: ${freshItem.status}), skipping reminder`
              );
            ...
            try {
              ...
              console.error(
                'Error sending direct reminder message:',
                directError
              );
              ...
              if (lens === 'quests') {
                try {
                  ...
                  console.log(
                    `Fallback reminder sent for quest ${freshItem.id}`
                  );
                } catch (remindError) {
                  console.error(
                    'Error in fallback reminder method:',
                    remindError
                  );
                }
              }
  ...
  async schedule(ctx) {
    try {
      ...
      if (showImages) {
        ...
        // For text messages, edit both text and markup
        const originalMessage = await this.quests.createMessage(
          quest,
          language
        );
        await ctx
          .editMessageText(originalMessage, {
            parse_mode: 'Markdown',
  ...
  async updateRecurringTask(taskId, updates) {
    try {
      ...
      console.log(
        'Updating recurring task:',
        taskId,
        'for holon ID:',
        task.holonId
      );
      if (task.chat) task[task.holonId] = task.chat;
      ...
      if (reminders && reminders.length > 0) {
        ...
        for (const reminder of reminders) {
          try {
            ...
            if (reminderDate <= now) {
              console.log(
                `Reminder ${reminder.id} for quest ${reminder.questId} has passed, removing it`
              );
            ...
            if (!reminder.holonId) {
              console.log(
                `Reminder ${reminder.id} has no valid holonId, removing it`
              );
              await this.deleteReminderRecord(reminder.id);
              continue;
            }
            ...
            try {
              ...
              if (!item) {
                console.log(
                  `${lens} item ${reminder.questId} for reminder ${reminder.id} no longer exists, removing reminder`
                );
                await this.deleteReminderRecord(reminder.id);
                continue;
              }
              ...
              if (item.status !== 'scheduled') {
                console.log(
                  `${lens} item ${reminder.questId} is no longer scheduled (status: ${item.status}), removing reminder ${reminder.id}`
                );
                await this.deleteReminderRecord(reminder.id);
                continue;
              }
              ...
              console.log(
                `Scheduling reminder ${reminder.id} for ${lens} ${reminder.questId} at ${reminderDate}`
              );
              ...
              const job = new CronJob(
                ...
                async () => {
                  try {
                    ...
                    console.log(
                      `Executing reminder for ${itemLens} ${reminder.questId}`
                    );

                    // Get fresh copy of the item
                    const freshItem = await this.db.get(
                      reminder.holonId,
                      itemLens,
                      reminder.questId
                    );
                    if (!freshItem) {
                      console.log(
                        `${itemLens} item ${reminder.questId} no longer exists, skipping reminder`
                      );
                      await this.deleteReminderRecord(reminder.id);
                      return;
                    }
                    ...
                    if (freshItem.status !== 'scheduled') {
                      console.log(
                        `${itemLens} item ${reminder.questId} is no longer scheduled (status: ${freshItem.status}), skipping reminder`
                      );
                      await this.deleteReminderRecord(reminder.id);
                      return;
                    }
                    ...
                    // Try direct message approach first
                    const itemType =
                      itemLens === 'events' ? 'Event' : 'Reminder';
                    try {
                      await this.bot.telegram.sendMessage(
                        reminder.holonId,
                        `${itemType}: "${freshItem.title}" is starting now!`,
                      ...
                      if (itemLens === 'quests') {
                        try {
                          await this.quests.remind(mockCtx, freshItem);
                          console.log(
                            `Fallback reminder sent for quest ${reminder.questId}`
                          );
                        } catch (remindError) {
                          console.error(
                            'Error in fallback reminder method:',
                    ...
                    console.error(
                      `Error executing reminder ${reminder.id}:`,
                      error
                    );
                  }
                },
              ...
              console.error(
                `Error verifying quest for reminder ${reminder.id}:`,
                error
              );
  ...
  async migrateRecurringLookup() {
    ...
    try {
      ...
      if (lookups && lookups.length > 0) {
        console.log(
          `Found ${lookups.length} recurring lookup records to migrate.`
        );
        ...
        console.log(
          `Successfully migrated ${migratedCount} recurring lookup records.`
        );
      } else {
