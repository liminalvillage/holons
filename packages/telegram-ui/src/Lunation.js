...
class Lunation {
  ...
  constructor(bot) {
    ...
    bot.command('fullmoon', ctx => {
      const fullMoon = lune.phase_hunt().full_date;
      const prefix = lune.phase().phase > 0.5 ? 'was' : 'will be';
      ctx.reply(
    ...
    bot.command('newmoon', ctx => {
      const newMoon = lune.phase_hunt().nextnew_date;
    ...
    bot.command('lunation', ctx => {
      const age = lune.phase().age.toFixed(0);
      ctx.reply(
    ...
    bot.command('prompt', ctx => {
      const age = lune.phase().age.toFixed(0);
      const prompt = prompts[age];
      ctx.reply(`\u{1F4DC} ${prompt}`);
    });
  }

  progress() {
    return lune.phase().age.toFixed(0);
  }
}
/******* lunation date***** */
