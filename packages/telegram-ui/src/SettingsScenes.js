...
export default class SettingsScenes {
  ...
  setupAdminScene() {
    this.adminScene.enter(async ctx => {
      ...
      if (ctx.callbackQuery) {
        ctx.scene.state.originalMessageId =
      ...
      await ctx.reply(
        i18next.t('settings_enter_admin_username', { lng: language })
      );
    });
    ...
    this.adminScene.on('message', ctx => {
      ...
      this.getLanguage(holonId).then(language => {
        ctx
          .reply(i18next.t('settings_send_text_only', { lng: language }))
  ...
  setupFederationScene() {
    this.federationScene.enter(async ctx => {
      ...
      await ctx.reply(
        i18next.t('settings_enter_federation_id', { lng: language })
      );
    });
    ...
    this.federationScene.on('text', async ctx => {
      ...
      try {
        ...
        if (!federationID) {
          await ctx.reply(
            i18next.t('settings_invalid_federation_id', { lng: language })
          );
          return;
        }
        ...
        if (!isNumeric && !isHex) {
          await ctx.reply(
            i18next.t('settings_invalid_federation_id_format', {
              lng: language,
              defaultValue:
                'Invalid holon ID format. Please enter a numeric ID (e.g., -1001234567890) or a hex address (e.g., 0x1234abcd).',
            })
          );
          return;
        }
        ...
        await this.db.keyManager.setupFederation(
          holonId.toString(),
        ...
        // Success message
        await ctx.reply(
          i18next.t('settings_federation_success', {
            lng: language,
        ...
        await ctx.reply(
          i18next.t('settings_federation_error', {
            lng: language,
            error: error.message,
          })
        );
  ...
  setupAddUserScene() {
    ...
    this.addUserScene.on('text', async ctx => {
      ...
      try {
        ...
        if (parts.length < 1) {
          await ctx.reply(
            i18next.t('settings_invalid_user_format', { lng: language }) ||
        ...
        if (!userId || isNaN(parseInt(userId))) {
          await ctx.reply(
            i18next.t('settings_invalid_user_id', { lng: language }) ||
        ...
        await ctx.reply(
          i18next.t('settings_error_adding_user', {
            lng: language,
            error: error.message,
  ...
  setupListPickerScene() {
    this.listPickerScene.enter(async ctx => {
      ...
      await ctx.reply(
        i18next.t('settings_select_option', { lng: language, type: title }),
        {
