...
import {
  createHolonBundle,
  createBundleContracts,
} from '../utils/holonOperations.js';
...
export default class Holons {
  ...
  constructor(bot, db, settings) {
    ...
    this.createHolonScene.enter(async ctx => {
      ...
      if (ctx.callbackQuery) {
        await ctx
          .editMessageText('Select a holon type to create:', {
            reply_markup: { inline_keyboard: inlineKeyboard },
          })
          .catch(error => console.error('Error editing message:', error));
      } else {
        await ctx.reply('Select a holon type to create:', {
          reply_markup: { inline_keyboard: inlineKeyboard },
        });
      }
    });
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: '✅ Yes',
                    callback_data: `confirm_holon_creation_${flavor}`,
                  },
    ...
    this.createHolonScene.action(/confirm_holon_creation_(.+)/, async ctx => {
      ...
      try {
        ...
        const holonName = holonId.toString();
    ...
    this.createHolonScene.action('cancel_create_holon', async ctx => {
      await ctx.answerCbQuery().catch();
      await ctx.editMessageText('Holon creation cancelled.', {
        reply_markup: {
          inline_keyboard: [
            [{ text: '◀️ Back to Menu', callback_data: 'holons_back' }],
          ],
    ...
    this.tokenBalanceScene.on('text', async ctx => {
      ...
      try {
        const contracts = await this.getHolonContracts(
          holonId,
          this.holonsContract,
          this.wallet
        );
        ...
        const { managed } = contracts.contracts;
        ...
        const contractBalance = await tokenContract.balanceOf(managed.address);
        ...
        const balances = await Promise.all(
          userIds.map(
            async userId =>
              await managed.contract.tokenBalance(userId, tokenAddress)
          )
        );
        ...
        const spliterAddress = (
          await this.getSplitterContract(holonIdNormalized)
        ).target;
    ...
    this.claimScene.enter(async ctx => {
      await ctx.reply(
        `Please enter your wallet address on ${this.network} to claim tokens:`
      );
    });
    ...
    this.claimScene.on('text', async ctx => {
      ...
      try {
        ...
        const holonAddress = managed.address;
        ...
        if (holonAddress === '0x0000000000000000000000000000000000000000') {
          await ctx.reply(
            'No holon exists for this chat. Create one first with /createholon'
          );
          await ctx.scene.leave();
          return;
        }
        ...
        return ctx.reply(
          `Transaction submitted. You will be notified when your claim is processed.`
        );
      } catch (error) {
    ...
    this.rewardScene.enter(async ctx => {
      await ctx.reply(
        'Please enter the token address and amount to reward members.\nFormat: [token address] [amount]'
      );
    });
    ...
    this.rewardScene.on('text', async ctx => {
      ...
      if (args.length < 2) {
        await ctx.reply(
          'Please provide both token address and amount.\nFormat: [token address] [amount]'
        );
        return;
      }
      ...
      try {
        ...
        const senderBalance = await this.provider.getBalance(
          this.wallet.address
        );
        console.log(
          'Sender balance:',
        ...
        console.log(
          'Zoned contract balance:',
          ethers.formatEther(zonedBalance),
          'ETH'
        );
        ...
        if (senderBalance < totalCost) {
          ...
          throw new Error(
            `Insufficient balance. Need ${neededEth} ETH but have ${haveEth} ETH`
          );
        }
        ...
        if (!isEthTransfer) {
          ...
          try {
            ...
            console.log(
              'Splitter address from the reward scene, approval: ',
              splitter.address
            );
            // Check current allowance for splitter contract only
            const splitterAllowance = await tokenContract.allowance(
              this.wallet.address,
              splitter.address
            );
            ...
            if (splitterAllowance < parsedAmount) {
              await ctx.reply(
                'Approving token transfer for splitter contract...'
              );
            ...
            throw new Error(
              `Failed to process token approval: ${error.message}`
            );
          }
        }
        ...
        const splitterTx = await splitter.contract.reward(
          tokenAddress,
          parsedAmount,
          txOptions
        );
        ...
        // Provide immediate feedback
        await ctx.reply(
          `Transaction submitted. You will be notified when the reward of ${ethers.formatUnits(parsedAmount, 18)} ${isEthTransfer ? 'ETH' : 'tokens'} is distributed.`
        );
      } catch (error) {
        console.error('Error in reward function:', error);
        await ctx.reply(
          'An error occurred while processing the reward: ' + error.message
        );
        await ctx.scene.leave();
      }
    });

    this.assignMemberToZoneScene = new Scenes.BaseScene(
      'assign_member_to_zone_scene'
    );
    this.addMemberScene = new Scenes.BaseScene('add_member_scene');
    this.addExternalHolonScene = new Scenes.BaseScene(
      'add_external_holon_scene'
    );
    ...
    const deploymentData = JSON.parse(
      fs.readFileSync('./contracts/deployment.json', 'utf-8')
    )[this.network];
    const holonsAddress = deploymentData.Holons;
    const holonsABI = JSON.parse(
      fs.readFileSync('./contracts/Holons.json', 'utf-8')
    ).abi;
  ...
  setupBotCommands() {
    ...
    this.bot.command('addtozone', async ctx =>
      this.handleAddToZoneCommand(ctx)
    );
    this.bot.command('setrewardfunction', async ctx =>
      this.handleSetRewardFunction(ctx)
    );
    this.bot.command('rewardpreview', async ctx =>
      this.handleRewardPreview(ctx)
    );
    ...
    this.bot.command('listmembers', async ctx => {
      const holonId = utils.getholonId(ctx);
      const holonName = `chat_${Math.abs(holonId)}`;
      const holon = await this.getManagedContract(holonName);
      const membersLength = await holon.getSize();
      const members = [];
      console.log('members length from /listmembers: ', membersLength);
      for (let i = 0; i < membersLength; i++) {
        const member = await holon.userIds(i);
        members.push(member);
      }
      if (membersLength > 0) {
        const holonIdNormalized = `chat_${Math.abs(holonId)}`;
    ...
    this.bot.command('assignzone', async ctx => {
      ...
      if (
        !zonedContract ||
        zonedContract.target === '0x0000000000000000000000000000000000000000'
      ) {
  ...
  setupCallbackHandlers() {
    ...
    this.bot.action('zone_prepare_move', async ctx => {
      await ctx
        ...
        .catch(e =>
          console.log(
            'Error answering CB query in zone_prepare_move:',
            e.message
          )
        );
      try {
        ...
        await ctx
          .reply('An error occurred while preparing to move a member.')
    ...
    this.bot.action('zone_prepare_remove', async ctx => {
      await ctx
        ...
        .catch(e =>
          console.log(
            'Error answering CB query in zone_prepare_remove:',
            e.message
          )
        );
      try {
        ...
        await ctx
          .reply('An error occurred while preparing to remove a member.')
          .catch(e => console.log('Reply error', e.message));
      }
    });
    ...
    this.bot.action(/zone_execute_move_([^_]+)_(\d+)_(\d+)/, async ctx => {
      ...
      await this.executeZoneMove(
        ctx,
        memberId,
        targetZoneTelegramIndex,
        originalZoneTelegramIndex
      );
    });
    ...
    this.bot.action('zone_add_external_holons_scene_enter', async ctx => {
      await ctx
        ...
        .catch(e =>
          console.log(
            'Error answering CBQ for add_external_holons_scene_enter:',
            e.message
          )
        );
    ...
    this.bot.action(/zone_confirm_remove_([^_]+)_(\d+)/, async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e =>
          console.log('CBQ Error in zone_confirm_remove:', e.message)
        );
      const memberId = ctx.match[1];
      const holonId = utils.getholonId(ctx);
      ...
      try {
        const zonedContract = await this.getZonedContract(holonIdNormalized);
        if (
          !zonedContract ||
          zonedContract.target === '0x0000000000000000000000000000000000000000'
        ) {
        ...
        await ctx
          .reply(
            `Transaction submitted to remove ${memberDisplay} from zone. You'll be notified upon completion.`
          )
          .catch(e =>
            console.log('Error replying for remove submission:', e.message)
          );
        ...
        await ctx
            ...
            `❌ Failed to remove ${memberDisplay}: ${error.message}`,
            {
              reply_markup: {
                inline_keyboard: [
                  [
                    {
                      text: '◀️ Back to Zone Management',
                      callback_data: 'holons_manage_zones_view',
                    },
                  ],
                ],
              },
          ...
          .catch(e =>
            console.log('Error editing message for remove failure:', e.message)
          );
      }
    });
    ...
    this.bot.action(/holons_(.+)/, async ctx => {
      await ctx
        ...
        .catch(e =>
          console.log(
            'Error answering CB query in main holons handler:',
            e.message
          )
        );
      ...
      if (
        !splitterContract ||
        splitterContract.target === '0x0000000000000000000000000000000000000000'
      ) {
        if (action !== 'create' && action !== 'back') {
          return ctx
            .editMessageText(
              'No holon exists for this chat. Create one first.',
              {
                reply_markup: {
                  inline_keyboard: [
                    [
            ...
            .catch(e =>
              console.log('Error editing message for no holon:', e.message)
            );
        }
      }
      switch (action) {
        ...
        case 'manage_zones_view':
          try {
            console.log(
              `Zone management view requested for holonId: ${holonId}`
            );
            const zonedContract =
              await this.getZonedContract(holonIdNormalized);
            if (
              !zonedContract ||
              zonedContract.target ===
                '0x0000000000000000000000000000000000000000'
            ) {
              console.log(`No zoned contract found for ${holonIdNormalized}`);
              return ctx
                .editMessageText(
                  'This holon does not have Zoned functionality.',
                  {
                    reply_markup: {
                      inline_keyboard: [
                        [{ text: '◀️ Back', callback_data: 'holons_back' }],
                ...
                .catch(e =>
                  console.log(
                    'Error editing message for no zoned functionality:',
                    e.message
                  )
                );
            }
            console.log(
              `Zoned contract found: ${zonedContract.target}, showing zone management view`
            );
            ...
            if (
              ...
              console.log(
                'Zone management view: message not modified, answering callback'
              );
              ...
              if (ctx.callbackQuery) {
                await ctx
                  .answerCbQuery()
                  .catch(e =>
                    console.log('CBQ Error for not modified:', e.message)
                  );
              }
            } else {
              // Real error occurred, try to provide feedback
              console.log(
                'Zone management view: real error occurred, editing message'
              );
              await ctx
                .editMessageText('Error loading zone management view.', {
                  reply_markup: {
                    inline_keyboard: [
                      [{ text: '◀️ Back', callback_data: 'holons_back' }],
                    ],
        ...
        case 'flow_management':
          try {
            await this.showFlowManagementMenu(ctx, true);
          } catch (error) {
            console.error('Error in flow_management handler:', error);
          }
          break;
        case 'back':
          try {
        ...
        default:
          ...
          await ctx
            ...
            .catch(e =>
              console.log('Error replying to unknown action:', e.message)
            );
      }
    });
    ...
    this.bot.action(/splitter_adj_live_(-?\d+)_(\d+)/, async ctx => {
      try {
        await ctx
          .answerCbQuery()
          .catch(e => console.log('Splitter Adjust CBQ Error:', e.message));
      } catch (e) {
        console.log('Error in answerCbQuery top for splitter_adj_live', e);
      }
      const adjustment = parseInt(ctx.match[1], 10);
      let previousInternalPercent = parseInt(ctx.match[2], 10);
      if (
        isNaN(previousInternalPercent) ||
        previousInternalPercent < 0 ||
        previousInternalPercent > 100
      ) {
      ...
      try {
        ...
        if (
          ...
          await ctx
            .editMessageText('Error: Splitter details unavailable.', {
              reply_markup: {
                inline_keyboard: [
                  [{ text: '◀️ Back', callback_data: 'holons_back' }],
                ],
        ...
        await ctx
          .editMessageText('Error: Could not access splitter info.', {
            reply_markup: {
              inline_keyboard: [
                [{ text: '◀️ Back', callback_data: 'holons_back' }],
              ],
      ...
      await this._updateSplitterManagementView(
        ctx,
        newInternalPercent,
        splitterAddress
      );
    ...
    this.bot.action(/splitter_conf_live_set_(\d+)/, async ctx => {
      try {
        await ctx
          .answerCbQuery()
          .catch(e => console.log('Confirm Split CBQ Error:', e.message));
      } catch (e) {
        console.log('Error in answerCbQuery top for confirm_split', e);
      }

      const finalInternalPercent = parseInt(ctx.match[1], 10);
      if (
        isNaN(finalInternalPercent) ||
        finalInternalPercent < 0 ||
        finalInternalPercent > 100
      ) {
        await ctx
          .editMessageText(
            'Error: Invalid percentage for setting split. Please try again.',
            {
              reply_markup: {
                inline_keyboard: [
                  [
                    {
                      text: 'Try Again',
                      callback_data: 'holons_manage_splitter_view',
                    },
                  ],
                ],
              },
      ...
      const holonId = utils.getholonId(ctx);
      const holonIdNormalized = `chat_${Math.abs(holonId)}`;
      // const internalId = `${holonIdNormalized}_managed`; // No longer needed for setContractSplit
      // const externalId = `${holonIdNormalized}_zoned`;   // No longer needed for setContractSplit

      await ctx
        .editMessageText(
          `Setting split to: Managed ${finalInternalPercent}% / Zoned ${finalExternalPercent}%...`
        )
        .catch(e => console.log('E:', e.message));
      try {
        const splitterContract =
        ...
        const tx = await this.executeTransaction(
          splitterContract,
          'setContractSplit',
          [finalInternalPercent, finalExternalPercent]
        );
        this.waitForTransaction(
          tx,
          ctx,
          `Split ratio set to Managed ${finalInternalPercent}% / Zoned ${finalExternalPercent}%!`
        );
        await ctx
          .reply('✅ Split ratio transaction submitted!')
          .catch(e => console.log('E:', e.message));
        await this.showHolonsMenu(ctx, false);
      } catch (error) {
        console.error(
          'Error executing setContractSplit in confirm handler:',
          error
        );
        await ctx
          .editMessageText(
            `❌ Error setting split: ${error.message}. Please try again.`,
            {
              reply_markup: {
                inline_keyboard: [
                  [
    ...
    this.bot.action(/zone_move_(\w+)_(\d+)_(\d+)/, async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e => console.log('CBQ Error in zone move:', e.message));
    ...
    this.bot.action(/reward_function_(.+)/, async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e => console.log('CBQ Error in reward function:', e.message));
      ...
      switch (action) {
        ...
        default:
          await ctx
            .editMessageText('Unknown reward function action.')
            .catch(e => console.log('Edit error:', e.message));
      }
    });
    ...
    this.bot.action(/poly_param_(.+)_(-?\d+)_(-?\d+)_(-?\d+)/, async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e => console.log('CBQ Error in poly param:', e.message));
    ...
    this.bot.action(/poly_save_(-?\d+)_(-?\d+)_(-?\d+)/, async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e => console.log('CBQ Error in poly save:', e.message));
    ...
    this.bot.action(/poly_(noop|info_.+)/, async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e => console.log('CBQ Error in poly noop:', e.message));
    ...
    this.bot.action(
      ...
      async function (ctx) {
        ...
        console.log(
          'Is this the main HolonsBot instance?',
          this === global.mainHolonsInstance
        );
        ...
        await ctx
          .answerCbQuery()
          .catch(e =>
            console.log('CBQ Error in member_sync_scores:', e.message)
          );
        await ctx
          .editMessageText('🔄 Syncing scores to contract... Please wait.')
          .catch(e => console.log('Edit error:', e.message));
        ...
        try {
          ...
          await ctx
            .editMessageText(`❌ Error syncing scores: ${error.message}`, {
              reply_markup: {
                inline_keyboard: [
                  [
                    {
                      text: '◀️ Back',
                      callback_data: 'holons_manage_members_view',
                    },
                  ],
                ],
              },
    ...
    this.bot.action('member_enter_add_scene', async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e =>
          console.log('CBQ Error in member_enter_add_scene:', e.message)
        );
    ...
    this.bot.action('member_prepare_remove', async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e =>
          console.log('CBQ Error in member_prepare_remove:', e.message)
        );
      try {
        ...
        await ctx
          .reply('Error loading remove mode.')
          .catch(e => console.log('Reply error:', e.message));
      }
    });
    ...
    this.bot.action(/member_confirm_remove_(.+)/, async ctx => {
      await ctx
        .answerCbQuery()
        .catch(e =>
          console.log('CBQ Error in member_confirm_remove:', e.message)
        );
      const memberId = ctx.match[1];
      await ctx
        .editMessageText(`➖ Removing member ${memberId}...`)
        .catch(e => console.log('Edit error:', e.message));
      ...
      try {
        ...
        await ctx
          .editMessageText(`❌ Error removing member: ${error.message}`, {
            reply_markup: {
              inline_keyboard: [
                [
                  {
    ...
    this.bot.action(
      /^member_(increment|decrement|increment10|decrement10)_(.+)$/,
      async ctx => {
        await ctx
          .answerCbQuery()
          .catch(e =>
            console.log('CBQ Error in member score adjustment:', e.message)
          );
        const holonId = utils.getholonId(ctx);
        const action = ctx.match[1];
        ...
        try {
          ...
          await ctx
            .reply('Error adjusting member score: ' + error.message)
            .catch(e => console.log('Reply error:', e.message));
        }
      }
    );
  ...
  async _updateSplitterManagementView(ctx, internalPercent, splitterAddress) {
    ...
    const keyboard = [
      [
        {
          text: '<<',
          callback_data: `splitter_adj_live_-10_${internalPercent}`,
        },
        { text: '<', callback_data: `splitter_adj_live_-1_${internalPercent}` },
        {
          text: `${internalPercent}% / ${externalPercent}%`,
          callback_data: 'noop',
        },
        { text: '>', callback_data: `splitter_adj_live_1_${internalPercent}` },
        {
          text: '>>',
          callback_data: `splitter_adj_live_10_${internalPercent}`,
        },
      ],
      [
        {
          text: '✅ Set This Split',
          callback_data: `splitter_conf_live_set_${internalPercent}`,
        },
      ],
      [
        {
          text: '◀️ Back to Flow Management',
          callback_data: 'holons_flow_management',
        },
      ],
    ];
      ...
      if (
        ...
        if (ctx.callbackQuery)
          await ctx
            ...
            .catch(e =>
              console.log(
                'Error answering CBQ for unmodified message:',
                e.message
              )
            );
      } else {
        console.error(
          'Error editing message in _updateSplitterManagementView:',
          error
        );
      }
    }
  ...
  async showFlowManagementMenu(ctx, edit = false) {
    ...
    try {
      splitterContract = await this.getSplitterContract(holonIdNormalized);
    } catch (error) {
      console.error(
        'Error checking holon bundle for flow management menu:',
        error
      );
    }
    ...
    if (!holonExists) {
      const message =
        'No Holon detected for this chat. Please create one first.';
      const keyboard = [
      ...
      if (edit || ctx.callbackQuery) {
        ...
        return ctx
          .reply(message, { reply_markup: { inline_keyboard: keyboard } })
          .catch(e => console.log('Error:', e.message));
      }
    }
    ...
    if (edit || ctx.callbackQuery) {
      try {
        ...
        if (
          ...
          if (ctx.callbackQuery)
            await ctx
              .answerCbQuery()
              .catch(e => console.log('E ans CBQ:', e.message));
          return;
        }
        console.log('Error editing flow management menu:', error);
        return ctx
          .reply(message, { parse_mode: 'Markdown', ...menuMarkup })
          .catch(e =>
            console.log(
              'Error replying to flow management menu fallback:',
              e.message
            )
          );
      }
      ...
      return ctx
        .reply(message, { parse_mode: 'Markdown', ...menuMarkup })
        .catch(e =>
          console.log('Error showing flow management menu:', e.message)
        );
    }
  }
  ...
  async showHolonsMenu(ctx, edit = false) {
    ...
    if (holonExists) {
      ...
      menuKeyboard.push(
        [
          { text: '🔄 Sync Scores', callback_data: 'holons_smart_sync' },
    ...
    if (edit) {
      try {
        ...
        return ctx
          .reply(message, { parse_mode: 'Markdown', ...menuMarkup })
          .catch(e =>
            console.log('Error replying to holons menu fallback:', e.message)
          );
      }
    } else {
      return ctx
        .reply(message, { parse_mode: 'Markdown', ...menuMarkup })
        .catch(e => console.log('Error showing holons menu:', e.message));
    }
  }
  ...
  async reward(ctx) {
    ...
    try {
      const holonAddress = await this.getManagedContract(holonName);
      ...
      const holon = new ethers.Contract(
        holonAddress,
        managed.default.abi,
        this.wallet
      );
      ...
      const data = holon.interface.encodeFunctionData('reward', [
        tokenAddress,
        amount,
      ]);
      ...
      // Provide immediate feedback
      await ctx.reply(
        `Transaction submitted. You will be notified when the flow of ${ethers.formatUnits(amount, 18)} tokens is distributed.`
      );
    } catch (error) {
      console.error('Error in reward function:', error);
      ctx.reply(
        'An error occurred while processing the flow: ' + error.message
      );
      await ctx.scene.leave();
    }
  }
  ...
  async ethBalance(ctx) {
    ...
    const balance = await holon.etherBalance(userID.toString());

    const holonIdNormalized = `chat_${Math.abs(holonId)}`;
    const spliterAddress = (await this.getSplitterContract(holonIdNormalized))
      .target;
  ...
  async tokenBalance(ctx) {
    ...
    const balances = await Promise.all(
      userIds.map(
        async userId => await holon.tokenBalance(userId, tokenAddress)
      )
    );
  ...
  async syncScore(ctx) {
    ...
    if (!this.expensesInstance) {
      console.error(
        '❌ Expenses instance not available in Holons.js for score calculation.'
      );
    ...
    const scoredUsers = await this.settings.calculateUserScores(
      usersObject,
      holonId,
      this.expensesInstance
    );
    console.log(
      `✅ Score calculation completed for ${scoredUsers.length} users`
    );
    ...
    const contractScores = users.map((user, i) => {
    ...
    console.log(
      'Final contract scores (BigInt, basis points):',
      contractScores.map(s => s.toString())
    );
    ...
    const maxPriorityFeePerGas =
      (feeData.maxPriorityFeePerGas * bufferMultiplier) / divisor;
    ...
    try {
      const setAppreciationFunc =
        holon.interface.getFunction('setAppreciation');
      console.log(
        'setAppreciation function signature:',
        setAppreciationFunc.inputs.map(i => `${i.name}: ${i.type}`)
      );

      const tx = {
        to: holon.target,
        data: holon.interface.encodeFunctionData('setAppreciation', [
          userids,
          contractScores,
        ]),
        gasLimit: 4000000,
      ...
      this.waitForTransaction(
        transactionResponse,
        ctx,
        `Successfully synced score for ${userids.length} members`
      );
      await ctx.reply(
        'Transaction submitted. You will be notified when the sync is completed.'
      );
  ...
  async waitForTransaction(tx, context, successMessage) {
    ...
    try {
        ...
        .then(async receipt => {
          if (receipt.status === 1) {
            if (successMessage && context) {
              try {
                await context.reply(
                  `✅ Transaction completed: ${successMessage} ${tx.hash}`
                );
              } catch (replyError) {
                // Context may have become invalid, just log it
                console.warn(
                  'Failed to send transaction success message:',
                  replyError.message
                );
              }
            }
            ...
            if (context) {
              try {
                ...
                console.warn(
                  'Failed to send transaction failure message:',
                  replyError.message
                );
              }
            }
      ...
      if (context) {
        try {
          await context.reply(
            `❌ Error setting up transaction: ${error.message}`
          );
        } catch (replyError) {
          console.warn(
            'Failed to send setup error message:',
            replyError.message
          );
        }
      }
  ...
  async createHolon(ctx) {
    ...
    const initialMessage = await ctx.reply(
      `🛠️ Creating Holon Bundle... Please wait.`
    );
    ...
    try {
      ...
      console.log('- holonId:', holonId, '(' + typeof holonId + ')');
      ...
      if (error.transactionHash)
        console.error('Failed Tx Hash:', error.transactionHash);
      ...
      await ctx.telegram
        ...
        .catch(editError =>
          console.error('Error editing message on failure:', editError)
        );
  ...
  async addMembersBundle(ctx) {
    ...
    const holonId = utils.getholonId(ctx);
    console.log('holonId from addMembers:', holonId);
    ...
    // const holonName = `${Math.abs(holonId)}`;
    // const holonName = holonId;
    const holonName = `chat_${Math.abs(holonId)}`;

    try {
      holonAddressManaged = await this.getManagedContract(holonName);
      console.log(
        'holonAddressManaged from addMembersBundle: ',
        holonAddressManaged.target
      ); // it's there, commenting it out
    } catch (error) {
    ...
    await ctx.reply(
      `Adding ${users.length} members to internal... Please wait.`
    );
    ...
    try {
      ...
      const data = holonAddressManaged.interface.encodeFunctionData(
        'addMembers',
        [userIds]
      );
      ...
      const addMembersFunc =
        holonAddressManaged.interface.getFunction('addMembers');
      if (addMembersFunc) {
      ...
      console.log(
        'managed holon address before sending: ',
        holonAddressManaged.target
      );
      ...
      this.waitForTransaction(
        tx,
        ctx,
        `Successfully added ${userIds.length} members`
      );

      // Provide immediate feedback
      results.push(
        `Transaction submitted. You will be notified when members are added.`
      );
    } catch (error) {
      console.error('Transaction error:', error);
  ...
  async addHolonsBundle(ctx) {
    ...
    const userIdsParams = utils.getParameters(ctx);
    ...
    try {
      ...
      console.log(
        'holonAddressZoned from addHolonBundle: ',
        holonAddressZoned.target
      );
    } catch (error) {
    ...
    try {
      ...
      const addMembersFuncInfo =
        holonAddressZoned.interface.getFunction('addMembers');
      ...
      const data = holonAddressZoned.interface.encodeFunctionData(
        'addMembers',
        args
      );
      ...
      this.waitForTransaction(
        tx,
        ctx,
        `Successfully initiated adding ${holonIdsToAddArray.length} holon(s) to external`
      );

      await ctx.reply(
        `Transaction submitted. You will be notified when holon(s) are added to external.`
      );
    } catch (error) {
  ...
  async sendCommand(_holonaddress, _command, _args) {
    const holon = await this.getHolonContract(_holonaddress);
    try {
  ...
  async newFlavor(_flavorname, _flavoraddress) {
    try {
      const tx = await this.holonsContract.newFlavor(
        _flavorname,
        _flavoraddress,
        {
          gasLimit: 3000000,
  ...
  async addFederationMember(holonAddress, userID, federationId) {
    try {
      ...
      if (flavor !== 'Zoned') {
        throw new Error(
          `This holon is of type "${flavor}" and does not support federation members. Only "Zoned" holons have federation functionality.`
        );
      }

      // Use the executeTransaction method for consistency with other functions
      console.log(
        'userId of the member that is added as federationMember:',
        String(userID)
      );
      console.log(
        'name of the member that is added as federationMember:',
        String(federationId)
      );
      const tx = await this.executeTransaction(
        holon,
  ...
  async showHolonTypes(ctx) {
    try {
      ...
      const flavorsList = flavors
        .map(flavor => `${flavorIcons[flavor] || '🔸'} ${flavor}`)
        .join('\n');
  ...
  async moveToZone(ctx) {
    ...
    try {
      const holonAddress = await this.holonsContract.toAddress(
        holonId.toString()
      );
      if (holonAddress === '0x0000000000000000000000000000000000000000') {
      ...
      if (flavor !== 'Zoned') {
        return ctx.reply(
          `This holon is of type "${flavor}" and does not support zones. Only external holons have zone functionality.`
        );
      }

      await ctx.reply(`Moving member to zone ${zoneNumber}... Please wait.`);

      const tx = await this.executeTransaction(holon, 'addToZone', [
        ctx.from.id,
        memberAddress,
        zoneNumber,
      ]);
      ...
      // Provide immediate feedback
      await ctx.reply(
        `Transaction submitted. You will be notified when the member is moved to zone ${zoneNumber}.`
      );
    } catch (error) {
  ...
  async showZones(ctx) {
    ...
    try {
      // let holonAddress = await this.holonsContract.toAddress(holonId.toString());
      // if (holonAddress === '0x0000000000000000000000000000000000000000') {
      //   return ctx.reply("No holon exists for this chat");
      // }
      ...
      try {
        ...
        for (let i = zoneCount; i >= 0; i--) {
          ...
          zoneMembers.push({
            ...
            members: [...members]
              .reverse()
              .map(member => userMap[member] || member), // Map to user tags
          });
        }
    ...
    try {
      ...
      switch (flavor) {
        ...
        case 'Splitter':
          return new ethers.Contract(
            holonAddress,
            splitter.default.abi,
            this.wallet
          );
        case 'Appreciative':
          return new ethers.Contract(
            holonAddress,
            appreciative.default.abi,
            this.wallet
          );
        default:
  ...
  async getSplitterContract(holonId) {
    try {
      // Convert holonId to string if it's not already
      const holonIdStr = holonId.toString();
      const holonAddress = await this.holonsContract.toAddress(holonIdStr);
  ...
  async getZonedContract(holonId) {
    try {
      // First get the Splitter contract
      const splitterContract = await this.getSplitterContract(holonId);

      if (!splitterContract) {
        return null;
      }
      const holonName = holonId + '_zoned';

      // Get the Zoned contract from the Splitter
      const zonedContractAddress =
        await splitterContract.contractsByType(holonName);
      const zonedContract = new ethers.Contract(
        zonedContractAddress,
        zoned.default.abi,
        this.wallet
      );
  ...
  async getManagedContract(holonId) {
    try {
      // First get the Splitter contract
      const splitterContract = await this.getSplitterContract(holonId);
      ...
      const holonName = holonId + '_managed';

      // Get the Managed contract from the Splitter
      const managedContractAddress =
        await splitterContract.contractsByType(holonName);
      const managedContract = new ethers.Contract(
  ...
  async setShares(ctx) {
    ...
    try {
      ...
      const tx = await this.executeTransaction(holon, 'setShares', [
        memberAddress,
        shares,
      ]);
      ...
      // Provide immediate feedback
      await ctx.reply(
        `Transaction submitted. You will be notified when shares are set for ${memberAddress}.`
      );
    } catch (error) {
  ...
  // async setSplit(userTags, percentages, holonId) {
  //   try {
  ...
  //     const holonAddress = await this.holonsContract.toAddress(holonId.toString());
  ...
  async setSplit(userTags, percentages, holonId) {
    // Added holonId, but might not be needed here
    ...
    if (
      ...
      userTags.length !== percentages.length
    ) {
      // Adjusted error message for clarity based on new input structure
      throw new Error(
        "Invalid input format. Ensure you provide pairs like 'internal 10 external 90'."
      );
    ...
    for (let i = 0; i < userTags.length; i++) {
      ...
      if (!key || isNaN(value)) {
        // Keep isNaN check just in case
        throw new Error(
          'Invalid format. Found non-number percentage or missing tag.'
        );
    ...
    if (
      typeof data.internal !== 'number' ||
      typeof data.external !== 'number'
    ) {
      throw new Error(
        "Both 'internal' and 'external' values must be provided."
      );
  ...
  async handleSetSplitCommand(ctx) {
    try {
      ...
      if (args.length % 2 !== 0 || args.length === 0) {
        ctx.reply(
          "Invalid format. Please provide pairs like 'internal 10 external 90'."
        );
        return;
      }
      ...
      for (let i = 0; i < args.length; i += 2) {
        ...
        if (isNaN(percentage)) {
          ctx.reply(
            `Invalid percentage value '${percentageStr}' for tag '${tag}'. Please provide numbers.`
          );
          return;
        }
      ...
      const validatedData = await this.setSplit(userTags, percentages, holonId);
      // send the transaction to splitter contract
      const holonAddress = await this.holonsContract.toAddress(
        holonId.toString()
      );
      const holon = await this.getHolonContract(holonAddress);
      const tx = await this.executeTransaction(holon, 'setSplit', [
        validatedData,
        percentages,
      ]);
      await tx.wait();
  ...
  async appreciateUsersByUsername(
    userTags,
    percentages,
    holonId,
    senderUserId
    ...
    try {
      ...
      for (let i = 0; i < userIds.length; i++) {
        try {
          const tx = await this.executeTransaction(holon, 'appreciate', [
          ...
          console.error('Transaction failed:', transactionError);
          throw new Error(
            'Failed to set appreciation due to transaction error.'
          );
        }
      }
    } catch (error) {
  ...
  async handleAppreciateCommand(ctx) {
    try {
      ...
      await this.appreciateUsersByUsername(
        usernames,
        percentages,
        holonId,
        userID
  ...
  async handleAddToZoneCommand(ctx) {
    ...
    try {
      ...
      const user = await this.getUserByTag(holonId, userTag);
      if (!user) {
        holonID = args[0];
      } else {
      ...
      // Provide immediate feedback
      await ctx.reply(
        `Transaction submitted. You will be notified when ${userTag} is added to zone ${telegramZone}.`
      );
    } catch (error) {
  ...
  async claim(ctx) {
    ...
    try {
      ...
      if (args.length < 1) {
        return ctx.reply(
          `Usage: /claim [your wallet address on ${this.network}]`
        );
      }
      ...
      const dataAdd = holon.interface.encodeFunctionData('addMember(string)', [
        userIdStr,
      ]);
      const dataClaim = holon.interface.encodeFunctionData('claim', [
        userIdStr,
        beneficiaryAddress,
      ]);
      ...
      this.waitForTransaction(
        addTx,
        ctx,
        `Added member! Tx hash: ${addTx.hash}`
      );
      ...
      if (!claimRcpt || claimRcpt.status !== 1) {
        // 🔺 extra null guard
      ...
      this.waitForTransaction(
        claimTx,
        ctx,
        `Claim successful! Tx hash: ${claimTx.hash}`
      );
  ...
  async listMembers(ctx) {
    const holonId = utils.getholonId(ctx);
    const holonIdNormalized = `chat_${Math.abs(holonId)}`;
    const managedContract = await this.getManagedContract(holonIdNormalized);
    let message = '';

    if (!managedContract) {
      message = 'Error: Could not find managed contract';
    } else {
      const membersLength = await managedContract.getSize();
      const members = [];
      for (let i = 0; i < membersLength; i++) {
        const member = await managedContract.userIds(i);
        members.push(member);
      }
      message = `━━━━━━━━━━━━━━━━━━━━━━\n`;
      if (membersLength > 0) {
  ...
  async showZoneManagementView(ctx, mode = 'view') {
    // Mode: 'view', 'prepare_move', 'prepare_remove'
    const holonId = utils.getholonId(ctx);
    const holonIdNormalized = `chat_${Math.abs(holonId)}`;
    ...
    try {
      ...
      for (let i = 0; i <= 5; i++) {
        ...
        const membersToDisplay =
          mode === 'prepare_remove' && i === 5 ? [] : zoneMembers;

        keyboard.push([
          {
            text: `Zone ${i} (${membersToDisplay.length})`,
            callback_data: `zone_header_${i}`,
          },
        ]);

        if (membersToDisplay.length > 0) {
          const memberButtonObjects = await Promise.all(
            membersToDisplay.map(async memberId => {
              ...
              try {
                ...
                console.log(
                  'holonName from a function utils.getHolonName inside ShowZoneManagementView: ',
                  holonName
                );
              } catch (error) {
                console.warn(
                  `Could not get holon name for ${memberId}:`,
                  error.message
                );
              ...
              const displayName =
                holonName === normalizedMemberId.toString()
                  ? holonName
                  : `${holonName} (${normalizedMemberId})`;
      ...
      try {
        ...
        const federatedWith = [...new Set([...inbound, ...outbound])];

        if (federatedWith.length > 0) {
          keyboard.push([
            {
              text: `🔗 Federated Holons (${federatedWith.length})`,
              callback_data: ' ',
            },
          ]);
          ...
          for (const federatedHolonId of federatedWith) {
            ...
            let holonName;
            try {
              holonName = await utils.getHolonName(
                this.db,
                federatedHolonId,
                ctx
              );
              ...
              console.warn(
                `Could not get holon name for ${federatedHolonId}:`,
                error.message
              );
            ...
            const displayName = holonName
              ? `${holonName} (${federatedHolonId})`
              : federatedHolonId.toString();
      ...
      if (mode === 'prepare_move') {
        keyboard.push([
          { text: '❌ Cancel Move', callback_data: 'holons_manage_zones_view' },
        ]);
      } else if (mode === 'prepare_remove') {
        keyboard.push([
          {
        ...
        keyboard.push([
          {
            text: '🔗 Add External Holon(s)',
            callback_data: 'zone_add_external_holons_scene_enter',
          },
        ]);
        keyboard.push([
        ...
        return ctx
          ...
          .catch(async err => {
            ...
            try {
              ...
              await ctx
                .answerCbQuery('Error updating view. Please try again.')
                .catch(e => {});
            } catch (e) {
              /*already handled or cannot answer*/
            }
      ...
      if (ctx.callbackQuery) {
        try {
          await ctx.answerCbQuery('Failed to load zone view.').catch(e => {});
        } catch (e) {}
      }
  ...
  async showMemberManagementView(ctx, edit = false, mode = 'view') {
    // Mode: 'view', 'prepare_remove'
    const holonId = utils.getholonId(ctx);
    const holonIdNormalized = `chat_${Math.abs(holonId)}`;
    ...
    try {
      ...
      if (
        !managedContract ||
        managedContract.target === '0x0000000000000000000000000000000000000000'
      ) {
        ...
        const membersLength = Number(await managedContract.getSize());
        const members = [];
        if (membersLength > 0) {
          for (let i = 0; i < membersLength; i++) {
            const memberId = await managedContract.userIds(i);
        ...
        if (members.length === 0) {
          ...
          console.log(
            'showMemberManagementView: understanding if the settings is properly initialized!',
            this.settings
          );
          ...
          if (this.expensesInstance) {
            try {
              ...
              console.error(
                'Error calculating scores with Settings method:',
                e
              );
          ...
          for (const memberId of members) {
            ...
            try {
              const memberScore = await managedContract.appreciation(
                memberId.toString()
              );
          ...
          {
            const adjusted = memberData.map(
              m => m.calculatedScore + (memberAdjustments[m.id] || 0)
            );
            const localShares = normalizeShares(adjusted);
            memberData.forEach((m, i) => {
              localShareById[m.id] = localShares[i];
            });
          }
          ...
          for (const member of memberData) {
            ...
            if (mode === 'prepare_remove') {
              ...
              const buttonText = `@${userMap[member.id] || member.id} ➖`;
              const callbackData = `member_confirm_remove_${member.id}`;
              keyboard.push([
              ...
              const contractPercentage =
                totalContractScore > 0
                  ? Math.round(
                      (member.contractScore / totalContractScore) * 100
                    )
                  : 0;
              ...
              keyboard.push([
                {
                  text: '<<',
                  callback_data: `member_decrement10_${member.id}`,
                },
                { text: '<', callback_data: `member_decrement_${member.id}` },
                {
                  text: `${adjustedScore} (${percentage}%)`,
                  callback_data: 'noop',
                },
                { text: '>', callback_data: `member_increment_${member.id}` },
                {
                  text: '>>',
                  callback_data: `member_increment10_${member.id}`,
                },
              ]);
      ...
      if (mode === 'prepare_remove') {
        ...
        keyboard.push([
          {
            text: '🔄 Sync Scores to Contract',
            callback_data: 'member_sync_scores',
          },
        ]);
        keyboard.push([
      ...
      if (edit || ctx.callbackQuery) {
        // Prefer edit if it's a callback query context
        return ctx
          .editMessageText(message, {
      ...
      await ctx
        .reply('Error displaying member management: ' + error.message)
        .catch(e => {});
      return;
    }
  }
  ...
  async smartSync(ctx) {
    ...
    try {
      ...
      if (
        ...
        await ctx
          .reply(
            '❌ Managed Holon contract not found. Cannot perform Smart Sync.'
          )
          .catch(e =>
            console.log('Error replying no managed contract:', e.message)
          );
        return;
      }
      ...
      if (!dbUsers || dbUsers.length === 0) {
        await ctx
          .reply('ℹ️ No users found in the database to sync.')
          .catch(e => console.log('Error replying no db users:', e.message));
      ...
      if (dbMemberCount !== contractMemberCount) {
        await ctx
          .reply(
            `ℹ️ Member count mismatch (DB: ${dbMemberCount}, Contract: ${contractMemberCount}). Adding/updating internal members...`
          )
          .catch(e =>
            console.log('Error replying member mismatch:', e.message)
          );
        ...
        await ctx
          .reply('✅ Member synchronization step submitted.')
          .catch(e =>
            console.log('Error replying member sync submitted:', e.message)
          );
      } else {
        await ctx
          .reply('✅ Member counts match. Proceeding to score sync.')
          .catch(e =>
            console.log('Error replying member counts match:', e.message)
          );
      }

      // 2. Sync Scores
      await ctx
        .reply('📊 Syncing scores...')
        .catch(e => console.log('Error replying syncing scores:', e.message));
      // syncScore sends its own replies regarding transaction submission
      await this.syncScore(ctx);
      await ctx
        .reply('✅ Score synchronization step submitted.')
        .catch(e =>
          console.log('Error replying score sync submitted:', e.message)
        );

      await ctx
        .reply(
          '🏁 Smart Sync process steps submitted. You will receive notifications for each transaction.'
        )
        .catch(e =>
          console.log('Error replying smart sync complete:', e.message)
        );
    } catch (error) {
      console.error('Error during Smart Sync:', error);
      await ctx
        .reply(`❌ Error during Smart Sync: ${error.message}`)
        .catch(e => console.log('Error replying smart sync error:', e.message));
    }
  }
  ...
  async setupMemberAddScene() {
    ...
    this.addMemberScene.on('text', async ctx => {
      ...
      if (!userIdsString) {
        await ctx.reply(
          'No User IDs provided. Please try again or type /cancel.'
        );
      ...
      const tempCtx = {
        ...ctx,
        message: { ...ctx.message, text: `/addmembers ${userIdsString}` },
      }; // Mock context for getParameters

      await ctx
        .reply(`➕ Adding provided User ID(s) to internal...`)
        .catch(e => console.log('E:', e.message));
    ...
    this.addMemberScene.command('cancel', async ctx => {
      await ctx
        .reply('Cancelled adding members.')
        .catch(e => console.log('E:', e.message));
      await ctx.scene.leave();
      await this.showHolonsMenu(ctx, false);
    });
  ...
  async removeMemberFromManagedHolon(ctx, memberIdToRemove) {
    ...
    try {
      ...
      this.waitForTransaction(
        tx,
        ctx,
        `Successfully submitted removal of member ${memberIdToRemove}`
      );
      return true; // Indicate submission success
    } catch (error) {
      console.error(
        `Error in removeMemberFromManagedHolon for ${memberIdToRemove}:`,
        error
      );
      await ctx
        .reply(
          `❌ Failed to initiate removal for member ${memberIdToRemove}: ${error.message}`
        )
        .catch(e => console.log('E:', e.message));
  ...
  async promptForTargetZone(ctx, memberId, originalZoneTelegramIndex) {
    await ctx
      .answerCbQuery()
      .catch(e =>
        console.log('Error answering CBQ in promptForTargetZone:', e.message)
      );
...
Select the TARGET zone:`;
    ...
    for (let i = 0; i <= 5; i++) {
      if (i === parseInt(originalZoneTelegramIndex)) {
        // parseInt to be safe
        keyboard.push([
          {
            text: `➡️ Zone ${i} (Current)`,
            callback_data: `zone_noop_current_${i}`,
          },
        ]);
      } else {
        keyboard.push([
          {
            text: `🎯 Zone ${i}`,
            callback_data: `zone_execute_move_${memberId}_${i}_${originalZoneTelegramIndex}`,
          },
        ]);
      }
    }
    ...
    try {
      ...
      await ctx
        .reply('Error displaying target zone selection.')
        .catch(e => console.log('Reply error', e.message));
    }
  }
  ...
  async setupExternalHolonAddScene() {
    this.addExternalHolonScene.enter(async ctx => {
      await ctx
        ...
        .catch(e =>
          console.log(
            'Error replying in addExternalHolonScene enter:',
            e.message
          )
        );
    });
    ...
    this.addExternalHolonScene.on('text', async ctx => {
      ...
      if (!holonIdsString) {
        await ctx
          .reply('No Holon IDs provided. Please try again or type /cancel.')
          .catch(e => console.log('E:', e.message));
        return ctx.scene.reenter();
      }
      ...
      await ctx
        .reply(`🔗 Adding ${holonIdsToAddArray.length} external Holon(s)...`)
        .catch(e => console.log('E:', e.message));
      ...
      try {
        ...
        if (
          ...
          await ctx
            .reply('Error: External contract not found for this chat.')
            .catch(e => console.log('E:', e.message));
          await ctx.scene.leave();
          return this.showZoneManagementView(ctx);
        }
        ...
        const data = zonedContract.interface.encodeFunctionData('addMembers', [
          senderUserId,
          holonIdsToAddArray,
        ]);
        ...
        this.waitForTransaction(
          tx,
          ctx,
          `Successfully submitted request to add ${holonIdsToAddArray.length} external Holon(s).`
        );
      } catch (error) {
        console.error('Error adding external Holons:', error);
        await ctx
          .reply(`❌ Failed to add external Holons: ${error.message}`)
          .catch(e => console.log('E reply err:', e.message));
      }
      await ctx.scene.leave();
    ...
    this.addExternalHolonScene.command('cancel', async ctx => {
      await ctx
        .reply('Cancelled adding external Holons.')
        .catch(e => console.log('E:', e.message));
      await ctx.scene.leave();
      await this.showZoneManagementView(ctx);
    });
  ...
  async executeZoneMove(
    ...
    await ctx
      .answerCbQuery()
      .catch(e =>
        console.log('Error answering CBQ in executeZoneMove:', e.message)
      );
    const holonId = utils.getholonId(ctx);
    ...
    await ctx
      .editMessageText(moveMessage, {
        reply_markup: {
          inline_keyboard: [[{ text: 'Processing...', callback_data: 'noop' }]],
        },
      })
      .catch(e =>
        console.log('Error editing message for move start:', e.message)
      );
    ...
    try {
      ...
      const solidityTargetZone = this.invertZone(
        parseInt(targetZoneTelegramIndex, 10)
      );

      console.log(
        `Executing move: sender=${senderUserId}, member=${memberId}, targetSolidityZone=${solidityTargetZone} (from Telegram Zone ${targetZoneTelegramIndex})`
      );
      ...
      const data = zonedContract.interface.encodeFunctionData('addToZone', [
        senderUserId,
        memberId,
        solidityTargetZone,
      ]);
      ...
      console.log(
        'move between the zones, solidityTargetZone',
        solidityTargetZone
      );
      ...
      await ctx
        .reply(
          `Transaction submitted to move ${memberDisplay} to Zone ${targetZoneTelegramIndex}. You'll be notified upon completion.`
        )
        .catch(e =>
          console.log('Error replying for move submission:', e.message)
        );
      // Refresh the main zone management view after submitting the transaction
      return this.showZoneManagementView(ctx);
    } catch (error) {
      ...
      await ctx
        ...
        .catch(e =>
          console.log('Error editing message for move failure:', e.message)
        );
  ...
  async displaySplitterManagementView(ctx) {
    ...
    try {
      ...
      if (
        ...
        splitterAddress = splitterContract.target;
        try {
          const internalPBigInt =
            await splitterContract.internalContractSplitPercentage();
          ...
          if (
            isNaN(internalPercent) ||
            internalPercent < 0 ||
            internalPercent > 100
          ) {
            console.warn(
              `Fetched initial internalPercent ${internalPercent} is invalid. Defaulting to 50.`
            );
            internalPercent = 50;
          }
        } catch (e) {
          console.log(
            `Initial percentage fetch using direct contract methods failed, defaulting to 50:`,
            e.message
          );
          internalPercent = 50;
        }
        ...
        await ctx
          .editMessageText('Splitter contract not found for this chat.', {
            reply_markup: {
              inline_keyboard: [
                [
                  {
          ...
          .catch(e =>
            console.log(
              'DisplaySplitterView Error Edit (no contract): ',
              e.message
            )
          );
        return;
      ...
      console.error(
        'Error getting splitter contract for displaySplitterView:',
        error
      );
      await ctx
        .editMessageText('Error accessing splitter details.', {
          reply_markup: {
            inline_keyboard: [
              [
                {
        ...
        .catch(e =>
          console.log(
            'DisplaySplitterView Error Edit (contract access): ',
            e.message
          )
        );
      return;
  ...
  async handleSetRewardFunction(ctx) {
    ...
    try {
      ...
      if (
        ...
        return ctx.reply(
          'This chat does not have a Zoned Holon. Only Zoned Holons support polynomial reward functions.'
        );
      }

      await ctx.reply(
        `Setting polynomial reward function parameters...\nFormula: ${a} * zone² + ${b} * zone + ${c}\nPlease wait.`
      );
      ...
      // Provide immediate feedback
      await ctx.reply(
        `Transaction submitted. You will be notified when the polynomial reward function is updated.`
      );
  ...
  async showRewardPreview(ctx, a, b, c, nzones = null) {
    try {
      ...
      if (totalWeight > 0) {
        for (let zone = 0; zone < nzones; zone++) {
          const percentage = ((zoneWeights[zone] / totalWeight) * 100).toFixed(
            2
          );
  ...
  async updatePolynomialParameterUI(ctx, a, b, c, isNew = false) {
    ...
    try {
      ...
      const chartPath = await this.generateZoneDistributionChart(
        a,
        b,
        c,
        holonId
  ...
  createPolynomialParameterKeyboard(a, b, c) {
    ...
    keyboard.push([{ text: `A: ${a}`, callback_data: `poly_info_a` }]);
    keyboard.push([
    ...
    keyboard.push([{ text: `B: ${b}`, callback_data: `poly_info_b` }]);
    keyboard.push([
    ...
    keyboard.push([{ text: `C: ${c}`, callback_data: `poly_info_c` }]);
    keyboard.push([
  ...
  async savePolynomialParameters(ctx, a, b, c) {
    ...
    try {
      ...
      await ctx.editMessageText(
        'Failed to save polynomial parameters: ' + error.message
      );
    }
  }
  ...
  async generateZoneDistributionChart(a, b, c, holonId) {
    if (!this.ui) {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          console.log(
            'Could not get nzones from contract, using default:',
            error.message
          );
        }
      }

      return await this.ui.getZoneDistributionChart(a, b, c, nzones, holonId);
    } catch (error) {
  ...
  async getSettings(holonId) {
    return await this.settings.getSettings(holonId);
  }
      ...
      for (const log of receiptBundle.logs) {
        try {
          ...
          if (parsedLog && parsedLog.name === 'HolonBundleCreated') {
            ...
            console.log(
              `Found Splitter address from event: ${splitterAddress}`
            );
            break;
          }
        } catch (e) {
  ...
  async createBundleContracts(
    ...
    try {
      ...
      console.log(
        'Transaction submitted for createZonedContract:',
        txZoned.hash
      );
  ...
  async getHolonContracts(holonId, holonsContract, wallet) {
    try {
      ...
      const splitterABI = JSON.parse(
        fs.readFileSync('./contracts/Splitter.json', 'utf-8')
      ).abi;
      const zonedABI = JSON.parse(
        fs.readFileSync('./contracts/Zoned.json', 'utf-8')
      ).abi;
      const managedABI = JSON.parse(
        fs.readFileSync('./contracts/Managed.json', 'utf-8')
      ).abi; // Added Managed ABI
      ...
      console.log(
        'getHolonContracts has a splitter address & contract: ',
        splitterAddress
      );
