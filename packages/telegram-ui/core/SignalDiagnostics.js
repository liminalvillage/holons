...
export default class SignalDiagnostics {
  ...
  registerDiagnosticCommands() {
    ...
    this.bot.command('reset_signals', async ctx => {
      ...
      if (!moduleName) {
        return ctx.reply(
          'Usage: /reset_signals <module_name>\nExample: /reset_signals Quests'
        );
      }

      const cleared = this.signalManager.clearModuleActions(moduleName);
      await ctx.reply(
        `Cleared ${cleared} signals for module ${moduleName}. Module will need to re-register its handlers.`
      );
    });
  ...
  generateRecommendations(diagnostics, validation) {
    ...
    if (validation.missing.length > 0) {
      recommendations.push({
        ...
        action:
          'Ensure all modules are properly initialized and registering their handlers',
      });
    }
    ...
    if (diagnostics.conflicts.length > 0) {
      recommendations.push({
        ...
        action:
          'Review conflicting patterns and use priority system or unique patterns',
      });
    }
    ...
    for (const [module, signals] of Object.entries(diagnostics.byModule)) {
      if (signals.length > 50) {
        recommendations.push({
          ...
          action:
            'Consider breaking down the module or optimizing signal patterns',
        });
      }
    }
