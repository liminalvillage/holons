...
export default class SignalManager {
  ...
  registerAction(pattern, handler, module = 'unknown', priority = 0) {
    ...
    if (this.registeredActions.has(patternKey)) {
      ...
      const conflictInfo = {
        ...
        resolution:
          effectivePriority > existing.priority ? 'new wins' : 'existing wins',
      };
      log.warn(
        `Signal conflict detected: ${patternKey} - ${existing.module} vs ${module} -> ${conflictInfo.resolution}`,
        conflictInfo
      );
  ...
  safeRegisterAction(module, pattern, handler, options = {}) {
    const { priority = 0, override = false, warnOnConflict = true } = options;
    ...
    if (existing && !override) {
      if (warnOnConflict) {
        log.warn(
          `Action already registered for ${patternKey} by ${existing.module}`
        );
      }
      return false;
    }
