...
const AVATAR_SERVER_URL =
  process.env.AVATAR_SERVER_URL || 'http://localhost:3000'; // Your main app (Server.js)
...
const server = http.createServer(sslOptions, (req, res) => {
  ...
  if (req.url.startsWith('/gun')) {
    ...
    proxy.web(
      req,
      res,
      { target: AVATAR_SERVER_URL, changeOrigin: true },
      err => {
...
server.on('upgrade', (req, socket, head) => {
  if (req.url.startsWith('/gun')) {
    console.log(
      `Proxying WebSocket upgrade for GUN_SERVER: ${GUN_SERVER_URL}${req.url}`
    );
    proxy.ws(
      req,
      socket,
      head,
      { target: GUN_SERVER_URL, ws: true, changeOrigin: true },
      err => {
    ...
    console.log(
      `Proxying WebSocket upgrade for AVATAR_SERVER: ${AVATAR_SERVER_URL}${req.url}`
    );
    ...
    proxy.ws(
      ...
      socket,
      head,
      { target: AVATAR_SERVER_URL, ws: true, changeOrigin: true },
      err => {
...
proxy.on('error', (err, req, res) => {
  ...
  if (res && !res.headersSent) {
    ...
    console.error(
      'Proxy error occurred without a response object (likely WebSocket).'
    );
  }
});
