  ...
  return {
    ...
    async federateHolon(
      ...
      if (!federationData.has(sourceHolon)) {
        federationData.set(sourceHolon, {
          federated: [],
          lensConfig: {},
          partnerNames: {},
        });
    ...
    async canWrite(holonId: string, lens: string, writerPubKey: string) {
      return {
        canWrite: writerPubKey === this.client.publicKey,
        reason: "owner",
        accessType: "owner",
      };
    },
    ...
    async shareLensKeys(partnerPubKey: string, lensNames?: string[]) {
      return {
        shared: lensNames?.length || 0,
        failed: 0,
        lenses: lensNames || [],
      };
    },
    hasLensKey(holonId: string, lensName: string) {
      return false;
    },
    getEncryptionStats() {
      return { ownKeyCount: 0, receivedKeyCount: 0, totalPartnerGrants: 0 };
    },

    // Registration
    async registerHolon(holonId: string, publicKey: string) {
      return true;
    },
    async lookupHolon(holonId: string) {
      return null;
    },
    ...
    close() {},
    metrics() {
      return { writes: 0, reads: 0, subscriptions: 0 };
    },
...
describe("HoloSphere2 Federation Lifecycle", () => {
  ...
  it("federateHolon creates a bidirectional federation with lens config", async () => {
    ...
    expect(federation!.lensConfig[partnerHolon].lenses).toEqual([
      "quests",
      "offers",
    ]);
  ...
  it("unfederateHolon removes the federation", async () => {
    ...
    expect((await hs.getFederation(myHolon))!.federated).toContain(
      partnerHolon,
    );
  ...
  it("federateHolon is idempotent (no duplicate entries)", async () => {
    ...
    const federation = await hs.getFederation(myHolon);
    expect(
      federation!.federated.filter((id) => id === partnerHolon),
    ).toHaveLength(1);
...
describe("Federation Request Store", () => {
  it("createIncomingRequest creates correct structure", async () => {
    const { createIncomingRequest } =
      await import("./lib/stores/federationRequests");
  ...
  it("createIncomingUpdate and createOutgoingUpdate", async () => {
    const { createIncomingUpdate, createOutgoingUpdate } =
      await import("./lib/stores/federationRequests");
...
describe("writeWithIdentity", () => {
  it("passes actingAs from activeHolonIdentity store", async () => {
    ...
    const result = await writeWithIdentity(mockHs as any, "holon1", "quests", {
      id: "q1",
      title: "Test",
    });
    expect(result).toBe(true);
  ...
  it("returns false and notifies on AuthorizationError", async () => {
    ...
    const { writeWithIdentity } = await import("./lib/holosphereWrite");
    const result = await writeWithIdentity(
      mockHs as any,
      "holon1",
      "quests",
      { id: "q1" },
...
describe("Lens Capability Utilities", () => {
  it("generateCapabilityId creates deterministic ID", async () => {
    const { generateCapabilityId } =
      await import("./lib/capabilities/lensCapability");
