...
export class HolonsContract {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "Failed to initialize real contracts, using fallbacks:",
        error,
      );
  ...
  async connect(signer: ethers.Signer): Promise<void> {
    try {
      ...
      if (
        ...
        console.warn(
          "Network does not support ENS, continuing without ENS resolution",
        );
        this.signer = signer;
  ...
  private async getContract(
    ...
    console.log("[HolonsContract] getContract:", {
      ...
      abiPreview:
        abi?.slice(0, 2).map((item: any) => item.name || item.type) || "empty",
      cacheKey: key.substring(0, 80),
    });

    if (!abi || abi.length === 0) {
      console.error(
        "[HolonsContract] WARNING: Empty ABI for contract at",
        address,
      );
    }
  ...
  private async executeTransaction(
    ...
    if (!contract.interface || !contract[method]) {
      ...
      throw new Error(
        `Method "${method}" not found on contract at ${contract.target}`,
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (feeError.code === "UNSUPPORTED_OPERATION") {
          console.warn(
            "Fee data not available on this network, using defaults",
          );
          feeData = null;
        } else {
          throw feeError;
  ...
  async setFactories(
    ...
    const holonsContract = await this.getContract(this.addresses.Holons, [
      ...this.contractABIs.Holons,
      {
        inputs: [
  ...
  async createHolonBundle(
    ...
    const tx = await this.createHolon(
      "Bundle",
      creatorUserId,
      name,
      steepnessValue,
  ...
  async deployBundleDirect(
    ...
    console.log(
      "[HolonsContract] BUNDLE_BYTECODE length:",
      BUNDLE_BYTECODE?.length || 0,
    );
    console.log(
      "[HolonsContract] BUNDLE_BYTECODE prefix:",
      BUNDLE_BYTECODE?.substring(0, 30) || "EMPTY",
    );
    ...
    if (!BUNDLE_BYTECODE || BUNDLE_BYTECODE.length < 100) {
      console.error(
        "[HolonsContract] BUNDLE_BYTECODE is missing or too short!",
      );
      throw new Error(
        "Bundle bytecode not available. Please run the deployment setup.",
      );
    }
  ...
  async getHolonBundle(holonName: string): Promise<HolonBundle | null> {
    try {
      console.log(
        `[HolonsContract] Looking for Bundle with name: ${holonName}`,
      );
      ...
      console.log(
        `[HolonsContract] Using Holons contract at address: ${this.addresses.Holons}`,
      );
      ...
      try {
        ...
        console.log(
          `[HolonsContract] toAddress(${holonName}) returned: ${bundleAddress}`,
        );
      } catch (error: any) {
        if (error.code === "BAD_DATA" && error.value === "0x") {
          console.log(
            `[HolonsContract] Bundle ${holonName} not found in contract (empty return)`,
          );
          return null;
        }
        console.log(
          `[HolonsContract] Bundle ${holonName} not found in toAddress mapping:`,
          error,
        );
        return null;
      }
      ...
      if (
        ...
        console.log(
          `[HolonsContract] Bundle ${holonName} has zero address: ${bundleAddress}`,
        );
        return null;
      }

      console.log(
        `[HolonsContract] Found Bundle ${holonName} at address: ${bundleAddress}`,
      );
      ...
      try {
        ...
        console.log(
          `[HolonsContract] Bundle parameters: steepness=${steepness}, nzones=${nzones}`,
        );
      } catch (readError) {
        console.warn(
          "[HolonsContract] Could not read Bundle parameters, using defaults:",
          readError,
        );
      }
  ...
  async addMembersToManaged(
    ...
    return this.executeTransaction(contract, "addMembers", [userIds], {
      gasLimit: 3000000,
    });
  }
  ...
  async addHolonsToZoned(
    ...
    return this.executeTransaction(contract, "addMembers", [holonIds], {
      gasLimit: 3000000,
    });
  }
  ...
  async setSteepness(
    ...
    return this.executeTransaction(contract, "setSteepness", [steepness], {
      gasLimit: 500000,
    });
  }
  ...
  async setNzones(
    ...
    return this.executeTransaction(contract, "setNzones", [nzones], {
      gasLimit: 500000,
    });
  }
  ...
  async setInteriorSplit(
    ...
    if (userIds.length !== percentages.length) {
      throw new Error(
        "userIds and percentages arrays must have the same length",
      );
    }
  ...
  async addMember(
    ...
    return this.executeTransaction(contract, "addMember", [userId], {
      gasLimit: 500000,
    });
  }
  ...
  async assignToZone(
    ...
    return this.executeTransaction(contract, "assignToZone", [userId, zone], {
      gasLimit: 500000,
    });
  }
  ...
  async getTokenBalances(
    ...
    for (const tokenAddress of tokenAddresses) {
      try {
        ...
        console.error(
          `Error getting balance for token ${tokenAddress}:`,
          error,
        );
      }
    }
  ...
  async checkRegistryConfiguration(): Promise<{
    ...
    try {
      ...
      try {
        ...
        if (
          !result.managedFactory ||
          result.managedFactory === "0x0000000000000000000000000000000000000000"
        ) {
      ...
      try {
        ...
        if (
          !result.zonedFactory ||
          result.zonedFactory === "0x0000000000000000000000000000000000000000"
        ) {
      ...
      try {
        result.splitterFlavor =
          await holonsContract.getFlavorAddress("Splitter");
        if (
          !result.splitterFlavor ||
          result.splitterFlavor === "0x0000000000000000000000000000000000000000"
        ) {
      ...
      console.error(
        "[HolonsContract] Error checking registry configuration:",
        error,
      );
    ...
    try {
      ...
      if (!config.managedFactory || !config.zonedFactory) {
        if (this.addresses.ManagedFactory && this.addresses.ZonedFactory) {
          ...
          result.errors.push(
            "Factory addresses not available in deployment config",
          );
        }
      }
      ...
      if (!config.splitterFlavor && this.addresses.SplitterFactory) {
        try {
          ...
          console.log(
            "[HolonsContract] Splitter flavor registered successfully",
          );
        } catch (e: any) {
          ...
          if (e.message?.includes("already exists")) {
            ...
            console.error(
              "[HolonsContract] Failed to register Splitter flavor:",
              e,
            );
          }
        }
