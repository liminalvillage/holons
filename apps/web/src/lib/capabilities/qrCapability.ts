...
export function generateCapabilityId(
  ...
  const itemPart = itemId
    ? `-${itemId.replace(/\s+/g, "_").substring(0, 20)}`
    : "";
 ...
 */
export function createQRCapability(
  options: CreateQRCapabilityOptions,
): QRCapabilityToken {
  // Verify the private key matches the holon ID (issuer must be holon owner)
  const derivedPubKey = bytesToHex(
    schnorr.getPublicKey(options.issuerPrivateKey),
  );
  if (derivedPubKey !== options.holonId) {
    throw new Error(
      "Private key does not match holon ID - only holon owner can create capabilities",
    );
  }
  ...
  const signature = signCapability(
...
export function validateCapability(
  ...
  if (!verifyCapabilitySignature(cap)) {
    return {
      valid: false,
      reason: "Invalid signature - capability may be tampered",
  ...
  if (cap.issuerPubKey !== cap.holonId) {
    return {
      valid: false,
      reason: "Issuer is not the holon owner",
      code: "ISSUER_NOT_OWNER",
    };
  }
  ...
  if (cap.holonId !== targetHolonId) {
    return {
      valid: false,
      reason: "Capability is for a different holon",
      code: "ISSUER_NOT_OWNER",
    };
  }
  ...
  if (
    ...
    !cap.allowedActions.includes(normalizedAction)
  ) {
    return {
      valid: false,
  ...
  if (
    targetLens &&
    cap.allowedLenses.length > 0 &&
    !cap.allowedLenses.includes(targetLens)
  ) {
    return {
      valid: false,
 ...
 */
export function decodeCapabilityFromUrl(
  encoded: string,
): QRCapabilityToken | null {
  try {
...
export function createCapabilitiesForCards(
  ...
  for (const card of cards) {
    ...
    if (!lens) {
      console.warn(
        `Unknown card type: ${card.type}, skipping capability generation`,
      );
      continue;
    }
