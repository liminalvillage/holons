...
function getNetworkName(): string {
  ...
  const serverNetwork =
    typeof process !== "undefined"
      ? process.env?.VITE_NETWORK_NAME ||
        process.env?.VITE_NETWORK ||
        process.env?.NETWORK
      : undefined;
  if (serverNetwork) {
    console.log(
      "[ContractLoader] Using network from server env:",
      serverNetwork,
    );
...
export const isDevelopmentMode = () => {
  ...
  return (
    !networkName || networkName === "localhost" || networkName === "hardhat"
  );
};
...
export function getDeploymentAddresses(): Record<string, string> {
  ...
  if (Object.keys(addresses).length === 0) {
    console.warn(
      `[ContractLoader] No deployments found for network: ${network}, falling back to localhost`,
    );
