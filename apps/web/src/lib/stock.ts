...
// our shelf beside the partners'. A port of the kiosk's `$lib/stock` so the
// two surfaces read one shelf the same way.
 ...
 * lens is subscribed without cross-space dedupe.
 */
...
export function normalizeLens(items: unknown): unknown[] {
  if (Array.isArray(items)) return items.filter(Boolean);
  if (items && typeof items === "object")
    return Object.values(items as Record<string, unknown>).filter(Boolean);
  return [];
}
