...
describe("normalizeLens", () => {
  it("accepts an array or a record map and drops holes", () => {
    expect(normalizeLens([1, null, 2])).toEqual([1, 2]);
    expect(normalizeLens({ a: 1, b: undefined })).toEqual([1]);
    expect(normalizeLens(null)).toEqual([]);
  });
});
