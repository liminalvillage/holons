...
export function parseICalText(
  ...
  try {
    ...
    const windowEnd =
      window?.end ?? new Date(now.getFullYear(), 11, 31, 23, 59, 59);
    ...
    vevents.forEach((vevent) => {
      try {
        ...
        if (event.isRecurring()) {
          ...
          const durationMs =
            icalTimeToDate(event.endDate).getTime() -
            icalTimeToDate(event.startDate).getTime();
          let skipped = 0;
          let emitted = 0;
          ...
          pushOccurrence(
            icalTimeToDate(event.startDate),
...
export function isValidICalUrl(url: string): boolean {
  try {
    ...
    return (
      protocol === "http:" || protocol === "https:" || protocol === "webcal:"
    );
  } catch {
    return false;
  }
