...
export function saveVisitedHolons(
  walletAddress: string | null,
  visitedHolons: VisitedHolon[],
) {
  try {
    const localStorageKey = walletAddress
      ? `visitedHolons_${walletAddress}`
      : "visitedHolons_anonymous";
...
export function loadVisitedHolons(
  walletAddress: string | null,
): VisitedHolon[] {
  try {
    const localStorageKey = walletAddress
...
export function saveClickedHolons(
  walletAddress: string | null,
  clickedHolons: ClickedHolon[],
) {
  try {
    const localStorageKey = walletAddress
      ? `clickedHolons_${walletAddress}`
      : "clickedHolons_anonymous";
...
export function loadClickedHolons(
  walletAddress: string | null,
): ClickedHolon[] {
  try {
    const localStorageKey = walletAddress
...
// Add clicked holon
export function addClickedHolon(
  walletAddress: string | null,
  holonId: string,
  holonName: string,
  source: "global" | "navigator",
...
export function addVisitedHolon(
  walletAddress: string | null,
  holonId: string,
  holonName: string,
  source: "global" | "navigator" | "personal" | "federation" = "personal",
) {
  try {
    const now = Date.now();
...
export function mergeHolonLists(
  personalHolons: PersonalHolon[],
  visitedHolons: VisitedHolon[],
  clickedHolons: ClickedHolon[],
): PersonalHolon[] {
