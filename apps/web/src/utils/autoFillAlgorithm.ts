...
export function autoFillWeekSchedule(input: AutoFillInput): AutoFillResult {
  ...
  availableRoles.forEach((role) => {
    ...
    const assignments: DayAssignment[] = weekDays.map((day, index) => {
      ...
      if (hasPermanent && role.participants) {
        const permanentUser =
          role.participants.find((p) => p.isPermanent) || role.participants[0];
        return {
          dayOfWeek: index,
          date: dateStr,
          users: [
      ...
      const existingAssignment = role.weekSchedule?.assignments?.find(
        (a) => a.date === dateStr,
      );
      if (
        existingAssignment &&
        existingAssignment.users &&
        existingAssignment.users.length > 0
      ) {
  ...
  for (const slot of slotsToFill) {
    ...
    // Update counts
    userAssignmentCounts.set(
      selectedUser.id,
      (userAssignmentCounts.get(selectedUser.id) || 0) + 1,
    );
    roleUserCounts
      .get(slot.roleId)!
      .set(
        selectedUser.id,
        (roleUserCounts.get(slot.roleId)!.get(selectedUser.id) || 0) + 1,
      );
    filledSlots++;
  }
  ...
  const userSummary = users
    ...
    .map((user) => {
      const totalAssignments =
        result.stats.userAssignmentCounts.get(user.id) || 0;
...
export function carryOverWeekSchedule(
  ...
  const newAssignments: DayAssignment[] = newWeekDays.map((day, index) => {
    const previousDayAssignment = role.weekSchedule!.assignments.find(
      (a) => a.dayOfWeek === index,
    );

    if (
      previousDayAssignment &&
      previousDayAssignment.users &&
      previousDayAssignment.users.length > 0
    ) {
      return {
        dayOfWeek: index,
