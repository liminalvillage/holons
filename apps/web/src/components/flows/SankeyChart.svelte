<script lang="ts">
  ...
  //
  // The kiosk twin of this file renders the same `SankeyLayout` — all the
  // arithmetic lives in `@holons/core/flows`, so the two surfaces cannot drift
  // apart no matter which one someone is looking at. Only the palette differs,
  // and this one has a pointer to answer to: hovering a bar or a ribbon opens a
  // tooltip, where the kiosk (a wall display, touched not hovered) opens a
  // modal on tap.
  //
  // The tooltip knows two things by itself — what a bar is worth, and its share
  // of the flow — and asks the parent for the rest through `nodeDetails` /
  // `linkDetails`, since only the parent knows whether it is looking at a
  // ledger or an allocation.
  //
  // Deliberately NOT the d3 `ConcentricZoneChart` next door: that one is the
  // editable on-chain control at /[id]/flow. This is a picture.
  //
  // The viewBox is measured from the container so one SVG unit is one CSS
  // pixel: ribbons fill the width and label text stays the size it claims.
  ...
  export let linkDetails:
    ...
    | null = null;
  /** Closing line of a bar's tooltip — say what clicking it will do. */
  export let hint = "";
  ...
  $: tipTop = Math.min(Math.max(8, py + 14), Math.max(8, height - tipHeight - 8));
    ...
    for (const group of byDepth.values()) {
      ...
      for (const n of sorted) {
        ...
        if (STRUCTURAL.has(n.kind ?? "")) {
          if (h >= oneLine(n) + 8) plan.set(n.id, { mode: "in", cy: centre(n) });
  ...
  >
        ...
        {#each nodes as node (node.id)}
          ...
          {:else if mode === "in2"}
            ...
            <text
              ...
              class:dim={litNodes && !litNodes.has(node.id)}
            >
              <tspan class="name" x={vx} y={vy - FONT_PX * 0.6}>{node.label}</tspan>
    ...
    {#if hoverNode || hoverLink}
      ...
      >
        ...
        {#if share != null}
          <div class="tip-share">{share}% of the flow shown</div>
        {/if}
        {#if rows.length}
          <dl>
...
<style>
  ...
  /* The dashboard ground is near-black, so ribbons need more opacity than the
     kiosk's paper equivalents to register at all. */
  .ribbon {
    fill: #94a3b8;
    opacity: 0.38;
  }
