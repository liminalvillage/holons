...
| Coop      | members from `users`, live demand bars, `type:'proposal'` quests with reputation-weighted + delegation-aware tallies (`@holons/core/governance`), vote delegation on the `delegations` lens, real treasury (settlement fees in, executed funding proposals out) |
