<script lang="ts">
  ...
  onMount(() => {
    // Global hook the widget invokes: data-onauth="onAkashaTelegramAuth(user)".
    (window as any).onAkashaTelegramAuth = (user: TelegramUser) =>
      setUser(user);
    ...
    if (botUsername && host) {
      ...
      s.setAttribute("data-onauth", "onAkashaTelegramAuth(user)");
      host.appendChild(s);
    }
  });
</script>
