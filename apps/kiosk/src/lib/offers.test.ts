...
describe("filterBoard", () => {
  it("narrows every lane by title or category and keeps the board on an empty query", () => {
    const board = buildOfferBoard({
      ...base,
      scale: { kind: "holon" },
      quests: [
        offer("Flour", "a", 5),
        { ...offer("Ladder", "a", 1), category: "tools" },
        need("Bread flour", "a", 3),
      ],
    });
    expect(filterBoard(board, " ")).toBe(board);
    const tools = filterBoard(board, "tools");
    expect(tools.supply.map((c) => c.key)).toEqual(["Ladder"]);
    expect(tools.demand).toEqual([]);
    expect(tools.matches).toEqual([]);
    const flour = filterBoard(board, "flour");
    expect(flour.supply.map((c) => c.key)).toEqual(["Flour"]);
    expect(flour.demand.map((c) => c.key)).toEqual(["Bread flour"]);
    expect(flour.matches).toHaveLength(1);
  });
});
