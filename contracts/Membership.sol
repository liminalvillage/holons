...
contract Membership {
   ...
   // address payable[] internal _contributors;
    ...
    function appreciate(address _memberaddress, uint8 _percentage)
        public
    {
        require (isMember[msg.sender], "Sender is not a member"); // validate sender is a Holon member
        require (isMember[_memberaddress], "Reciever is not a member"); // validate receiver is a Holon member
   ...
   /// @dev Resets appreciation of the caller
    ...
    function upgradeToMember(address _memberaddress)
        public
    {
        require (msg.sender == lead,"Only the lead can upgrade membership");
        isMember[_memberaddress] = true;
    }
    ...
    /// @notice only the holon lead can call this function
