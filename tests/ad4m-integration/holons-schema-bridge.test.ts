...
describe.skipIf(!EXECUTOR_EXISTS)("Holons Schema Bridge", () => {
  ...
  // =========================================================================
  // Hand-Crafted vs Bridged Model Comparison
  // =========================================================================

  describe("Hand-Crafted vs Bridged Models", () => {
    it("should compare SDNA output between hand-crafted and bridged quest models", async () => {
      const { Quest } = await import("../../src/lib/ad4m/models/index");
      const { inspectBridgedSDNA, createBridgedSchemas } = await import(
        "../../src/lib/ad4m/schema-bridge"
      );

      // Get hand-crafted SDNA
      let handCraftedSDNA: { sdna: string; name: string } | null = null;
      try {
        handCraftedSDNA = inspectBridgedSDNA(Quest);
      } catch {
        // Hand-crafted models may not have generateSDNA
        console.log(
          "Note: Hand-crafted Quest model uses decorators, not generateSDNA"
        );
      }
      ...
      if (questSchema) {
        try {
          const bridgedSDNA = inspectBridgedSDNA(questSchema.modelClass);
          expect(bridgedSDNA).toBeTruthy();
          expect(bridgedSDNA.name).toBe("QuestSchema");
          console.log("Bridged SDNA name:", bridgedSDNA.name);
          console.log(
            "Bridged SDNA length:",
            bridgedSDNA.sdna?.length || 0,
            "chars"
          );
        } catch (err) {
          console.log("Bridged SDNA inspection not available:", err);
        }
      }
    ...
    it("should show mapping metadata for hand-crafted models", async () => {
      const { Quest, HolonSettings, HolonMember } = await import(
        "../../src/lib/ad4m/models/index"
      );
      const { getSchemaMapping } = await import(
        "../../src/lib/ad4m/schema-bridge"
      );

      for (const ModelClass of [Quest, HolonSettings, HolonMember]) {
        try {
          const mapping = getSchemaMapping(ModelClass);
          console.log(
            `${(ModelClass as any).name || "Model"} metadata:`,
            JSON.stringify(mapping, null, 2).slice(0, 200) + "..."
          );
          expect(mapping).toBeTruthy();
        } catch (err) {
          console.log(
            `Metadata not available for ${(ModelClass as any).name}:`,
            err
          );
        }
      }

      console.log("✅ Schema mapping metadata inspected");
    });
  });
