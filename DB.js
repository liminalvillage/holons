...
class DB {
    ...
    async open(table) {
        ...
        if (!this.preloadedDB[table]) {
            if (this.db === 'gun')
                this.preloadedDB[table] = await this.gun.get(this.dbName).get(table)
            if (this.db === 'orbit') {
    ...
    async addGunDB(table, data) {
        this.gun.get(this.dbName).get(table).get(data.id).put(JSON.stringify(data)) //todo store entire object
    ...
    async getAllGunDB(table) {
        ...
        return new Promise((resolve) => {
             this.gun.get(this.dbName).get(table).once((data, key) => {
            if (!data) {
                resolve(null);
            } else {
